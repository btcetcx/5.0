  ...
  return [
    {
      ...
      icon: 'line-folder',
      children: [
        { key: 'internal-rd', label: '内部研发', code: 'RD-INTERNAL', desc: '企业内部立项研发', count: 18 },
        { key: 'product-rd', label: '产品研发', code: 'RD-PRODUCT', desc: '新产品或产品迭代研发', count: 24 },
      ],
    },
    {
      ...
      icon: 'line-folder',
      children: [
        { key: 'system-upgrade', label: '系统改造', code: 'ENG-SYSTEM', desc: '系统升级与流程改造', count: 12 },
        { key: 'automation-engineering', label: '自动化工程', code: 'ENG-AUTO', desc: '产线与设备自动化工程', count: 9 },
      ],
    },
    {
      ...
      icon: 'line-folder',
      children: [
        { key: 'school-enterprise', label: '校企合作', code: 'CO-SCHOOL', desc: '校企联合研发项目', count: 6 },
        { key: 'joint-innovation', label: '联合创新', code: 'CO-INNOVATION', desc: '外部伙伴联合创新', count: 8 },
      ],
    },
  ];
}
...
function buildProductDetail(row: Record<string, any>) {
  ...
  return baseDetail('products', row, row.name || '智能温湿度传感器', [
    ...
    inbound: [table('product-quality', '质量记录', ['序号', '售后主题', '售后单号', '售后类型', '售后数量'], [
      ['1', '智能温湿度传感器现场校准', 'AS-202605-018', '现场服务', '3'],
      ['2', '外观划伤异常', 'QC-EX-202605-007', '质检异常', '12'],
    ])],
...
export function getRdCreateConfig(module: RdModule): RdCreateConfig {
  ...
  const configs: Record<RdModule, RdCreateConfig> = {
    ...
    projects: {
      ...
      sections: [
        { title: '基本信息', fields: [
          ...
          { key: 'category', label: '项目分类', type: 'categoryPicker', categoryKind: 'project', required: true, placeholder: '请选择项目分类' },
    ...
    products: {
      ...
      subTables: [{ key: 'specs', title: '新增型号', addText: '新增型号', columns: [{ key: 'model', title: '型号编码' }, { key: 'spec', title: '规格名称' }, { key: 'remark', title: '备注' }, { key: 'enabled', title: '启用', type: 'switch' }], rows: [
    ...
    materials: {
      ...
      sections: [{ title: '基础信息', fields: [
        ...
        { key: 'state', label: '物料状态', type: 'select', options: ['启用', '禁用', '停止采购'], defaultValue: '启用' },
      ] }],
      tabs: [
