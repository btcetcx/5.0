...
export const qcInspectionStatuses = ['待报检', '抽样中', '巡检中', '待判定', '待出货检验', '合格待入库', '返工待复检', '特采评审', '拒收退回', '已放行'];
...
export function updateQcStandard(id: string, patch: Partial<QcStandard>): Promise<QcStandard | undefined> {
  const index = standards.findIndex((item) => item.id === id || item.code === id);
  if (index < 0) return Promise.resolve(undefined);
  standards[index] = { ...standards[index], ...patch, tone: toneByStatus(patch.status || standards[index].status) };
  return Promise.resolve({ ...standards[index] });
}
