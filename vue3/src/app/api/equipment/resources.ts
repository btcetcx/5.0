...
export const repairStatusOptions = ['待派工', '维修中', '待验收', '返修中', '已完成', '已关闭'];
...
export function toneByStatus(status = ''): EquipmentTone {
  ...
  if (['待执行', '待派工', '待验收', '保养中', '维修中', '返修中', '已预警', '低库存', '执行中', '点检中'].includes(status)) return 'yellow';
...
export function createRepairOrder(payload: Partial<RepairOrder>) {
  ...
  const row: RepairOrder = {
    ...
    previousEquipmentStatus: payload.previousEquipmentStatus || asset.status,
    impactScope: payload.impactScope || '单台设备',
    urgency: payload.urgency || '中',
    faultDesc: payload.faultDesc || payload.repairDetail || '',
    repairDetail: payload.repairDetail || payload.faultDesc || '',
...
export function createInspectionRecord(payload: Partial<InspectionRecord>) {
  ...
  const row: InspectionRecord = {
    ...
    inspectionCycle: payload.inspectionCycle || '',
...
export function generateMaintenanceExecution(planId: string) {
  ...
  if (asset) {
    plan.previousEquipmentStatus = asset.status;
    asset.status = '保养中';
  }
  const execution: MaintenanceExecution = {
    ...
    planId: plan.id,
    previousEquipmentStatus: asset?.status === '保养中' ? plan.previousEquipmentStatus : asset?.status,
    executedAt: nowText(),
...
export function completeMaintenance(planId: string, spareUsage: SpareUsage[] = [], payload: Partial<MaintenanceExecution> = {}) {
  ...
  if (asset) {
    ...
    const restoreStatus = currentExecution.previousEquipmentStatus || plan.previousEquipmentStatus;
    asset.status = restoreStatus || (asset.status === '保养中' ? '运行中' : asset.status);
  }
...
function generateExecutionForComplete(plan: MaintenancePlan): MaintenanceExecution {
  return {
    ...
    code: `ME-202605-${String((plan.executions?.length || 0) + 1).padStart(3, '0')}`,
    planId: plan.id,
    previousEquipmentStatus: plan.previousEquipmentStatus,
    executedAt: nowText(),
    executor: plan.ownerName,
    result: '已完成',
    exception: '',
...
export function submitRepairResult(id: string, payload: Partial<RepairOrder>) {
  ...
  if (!['维修中', '返修中'].includes(order.status)) return Promise.resolve({ ok: false, message: '当前状态不能提交维修处理' });
  ...
  if (!stockCheck.ok) return Promise.resolve(stockCheck);
  const result = payload.result || '已修复';
  const needsRework = result === '需复修';
  Object.assign(order, {
    ...
    status: needsRework ? '返修中' : '待验收',
    acceptanceStatus: needsRework ? '无需验收' : '待验收',
  });
  return Promise.resolve({ ok: true, message: needsRework ? '维修处理已记录，任务进入返修中' : '维修处理已提交，进入待验收' });
}
...
function consumeSpares(usages: SpareUsage[], sourceCode: string, equipmentName: string, requestType: string) {
  const totals = new Map<string, { qty: number; usage: SpareUsage; availableQty: number; name: string }>();
  for (const usage of usages) {
    ...
    const key = spare.id;
    const current = totals.get(key) || { qty: 0, usage, availableQty: spare.availableQty, name: spare.name };
    current.qty += qty;
    totals.set(key, current);
  }
  for (const item of totals.values()) {
    if (item.qty > item.availableQty) return { ok: false, message: `${item.name}库存不足，不能负库存` };
  }
  for (const usage of usages) {
    const spare = spares.find((item) => item.id === usage.spareId || item.code === usage.spareCode);
    if (!spare) continue;
...
export function getWorkbenchSummary() {
  ...
  return Promise.resolve({
    cards: [
      ...
      { key: 'accept', label: '维修验收', value: repairs.filter((item) => item.status === '待验收').length },
