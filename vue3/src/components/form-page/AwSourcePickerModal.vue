...
import type { SourcePickerBatch, SourcePickerCategory, SourcePickerConfirmPayload, SourcePickerRow } from './types';
...
const props = withDefaults(
  defineProps<{
    ...
    batchTextMap?: Record<string, Partial<BatchPanelText>>;
    childBatchText?: Partial<BatchPanelText>;
...
const visibleBatches = computed(() => selectedSourceRows.value.flatMap((row) =>
  (props.batches[row.code] || []).map((batch) => ({ ...batch, sourceCode: batch.sourceCode || row.code, sourceType: batch.sourceType || row.cat })),
));
...
const defaultBatchPanelText = computed<BatchPanelText>(() => props.batchMode === 'workOrder'
  ? {
      title: '工单列表',
      no: '工单编号',
      date: '工单类型',
...
const batchPanelText = computed<BatchPanelText>(() => ({
  ...defaultBatchPanelText.value,
  ...(pickedSource.value ? props.batchTextMap[pickedSource.value.cat] : {}),
}));
const childBatchPanelText = computed<BatchPanelText>(() => ({
  ...defaultBatchPanelText.value,
  title: '工序列表',
  no: '工序号',
  date: '工序类型',
          ...
          <table class="aw-table">
            ...
            <tbody>
              ...
              <tr v-if="!filteredRows.length">
                <td colspan="6" class="aw-source-empty-cell">{{ currentCustomer ? '当前客户暂无该类型来源' : '请先选择客户后再选择来源' }}</td>
              </tr>
            </tbody>
          </table>
            ...
            <table class="aw-table">
              ...
              <tbody>
                ...
                <tr v-if="!visibleBatches.length">
                  <td colspan="7" class="aw-source-empty-cell">{{ batchPanelText.empty }}</td>
                </tr>
              </tbody>
            </table>
