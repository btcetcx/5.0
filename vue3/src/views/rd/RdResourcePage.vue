        ...
        <div v-if="isProjectDetail" class="rd-project-detail">
          ...
          <template v-else-if="activeDetailTab === 'members'">
            <div class="rd-project-action-row">
              <button class="aw-btn" type="button" @click="openProjectMemberPicker">新增成员</button>
              <button class="aw-btn danger" type="button" @click="detailMessage = '请选择成员后再移除'">移除成员</button>
            </div>
          ...
          <template v-else-if="activeDetailTab === 'materials'">
            <div v-if="!projectHasBom" class="rd-project-empty">
              ...
              <div>
                <button class="aw-btn primary" type="button" @click="openProjectModal('bomRef')">引用BOM</button>
                <button class="aw-btn" type="button" @click="openProjectModal('bomAdd')">添加BOM</button>
                <button class="aw-btn" type="button" @click="openProjectModal('bomImport')">导入Excel</button>
              </div>
            </div>
            <template v-else>
              <div class="rd-project-action-row">
                <button class="aw-btn" type="button" @click="openProjectModal('bomRef')">引用BOM</button>
                <button class="aw-btn" type="button" @click="openProjectModal('bomAdd')">添加BOM</button>
                <button class="aw-btn" type="button" @click="openProjectModal('bomImport')">导入Excel</button>
                <button :class="['aw-btn', projectBomLocked ? '' : 'primary']" type="button" @click="lockProjectBom">{{ projectBomLocked ? '已锁定' : '锁定BOM' }}</button>
          ...
          <template v-else-if="activeDetailTab === 'process'">
            <div v-if="!projectHasProcess" class="rd-project-empty">
              ...
              <div>
                <button class="aw-btn primary" type="button" @click="openProjectModal('processRef')">选择工艺流程</button>
                <button class="aw-btn" type="button" @click="openProjectModal('processAdd')">添加工艺流程</button>
              </div>
            </div>
            <template v-else>
              <div class="rd-project-action-row">
                <button class="aw-btn" type="button" @click="openProjectModal('processAdd')">添加工艺流程</button>
                <button :class="['aw-btn', projectProcessLocked ? '' : 'primary']" type="button" @click="lockProjectProcess">{{ projectProcessLocked ? '已锁定' : '锁定工艺' }}</button>
          ...
          <template v-else-if="activeDetailTab === 'quote'">
            <div v-if="!projectQuoteRows.length" class="rd-project-empty">
              ...
              <div><button class="aw-btn primary" type="button" @click="openProjectModal('quoteAdd')">新增成本项</button></div>
            </div>
            <template v-else>
              <div class="rd-project-action-row">
                <button class="aw-btn primary" type="button" @click="openProjectModal('quoteAdd')">新增成本项</button>
                <button :class="['aw-btn', projectQuoteConfirmed ? '' : 'primary']" type="button" @click="lockProjectQuote">{{ projectQuoteConfirmed ? '已锁定报价' : '锁定报价' }}</button>
              </div>
              <detail-table :columns="['序号', '成本项', '来源说明', '金额', '状态']" :rows="projectQuoteTableRows" />
              <div class="rd-project-metrics">
                <div><span>当前报价金额</span><strong>{{ money(projectQuoteBaseAmount) }}</strong></div>
                <div>
                  <span>调整金额</span>
                  <input v-model="projectQuoteAdjustAmount" class="aw-input aw-num" :disabled="projectQuoteConfirmed" />
                </div>
                <div class="primary"><span>实际报价金额</span><strong>{{ money(projectQuoteActualAmount) }}</strong></div>
              </div>
            </template>
          </template>
          ...
          <template v-else-if="activeDetailTab === 'purchase'">
            <div v-if="!projectHasPurchase" class="rd-project-empty">
              ...
              <div><button :class="['aw-btn', projectQuoteConfirmed ? 'primary' : '']" type="button" :disabled="!projectQuoteConfirmed" @click="startProjectPurchase">发起采购</button></div>
            </div>
            <detail-table v-else :columns="['序号', '采购单号', '来源', '采购金额', '状态', '进度']" :rows="projectPurchaseRows" />
            <div v-if="!projectQuoteConfirmed" class="rd-project-hint">锁定报价后才可以发起采购。</div>
          </template>
          ...
          <template v-else-if="activeDetailTab === 'production'">
            <div v-if="!projectHasProduction" class="rd-project-empty">
              ...
              <div><button :class="['aw-btn', projectQuoteConfirmed ? 'primary' : '']" type="button" :disabled="!projectQuoteConfirmed" @click="startProjectProduction">下单生产需求</button></div>
            </div>
            <detail-table v-else :columns="['序号', '生产需求号', '来源', '需求数量', '状态', '进度']" :rows="projectProductionRows" />
            <div v-if="!projectQuoteConfirmed" class="rd-project-hint">锁定报价后才可以下单生产需求。</div>
          </template>
          ...
          <template v-else-if="activeDetailTab === 'expense'">
            ...
            <div class="rd-project-action-row spread">
              <span class="rd-muted">{{ projectCompleted ? '项目已完成，不可新增成本项目。' : '可补充差旅、测试、外协等项目执行成本。' }}</span>
              <button class="aw-btn primary" type="button" :disabled="projectCompleted" @click="openProjectModal('projectCostAdd')">新增成本项目</button>
            </div>
  ...
  <div v-if="projectModal" class="aw-mask" @click="closeProjectModal">
    <div :class="['aw-modal', { lg: ['bomRef', 'bomAdd', 'processRef'].includes(projectModal), 'rd-project-route-modal': projectModal === 'processAdd' }, 'rd-project-modal']" @click.stop>
      ...
      <div class="foot">
        ...
        <button class="aw-btn primary" type="button" :disabled="projectModal === 'projectCostAdd' && projectCompleted" @click="confirmProjectModal">{{ projectModalPrimaryText }}</button>
      </div>
    </div>
  </div>
...
const detailToolbarActions = computed<DetailAction[]>(() => {
  ...
  return detailRecord.value?.actions || [];
});
...
const projectCompleted = computed(() => detailRecord.value?.statusText === '已完成');
...
function startProjectPurchase() {
  if (!projectQuoteConfirmed.value) return;
