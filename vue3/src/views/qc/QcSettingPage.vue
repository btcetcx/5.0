  ...
  <aw-setting-page>
    ...
    <section v-else-if="currentKey === 'strategies'" class="aw-form-card aw-policy-card">
      <div class="aw-policy-head">
        <strong>{{ qcPolicyConfig.title }}</strong>
        <span>用于控制业务逻辑、审批触发、转单回写和异常拦截规则</span>
      </div>
      <div class="aw-tabs">
        <button
          v-for="tab in qcPolicyConfig.tabs"
          :key="tab.k"
...
const qcApprovalOptions = ['检验方案发布审批流程', '让步放行审批流程'];
const qcPolicyConfig = ref({
  title: '质检策略设置',
  tabs: [
    {
      k: 'archive',
      label: '检验执行',
      rows: [
        { title: '检验方案发布审批流程', sub: '新增检验标准提交后按指定审批流处理；可用于校验检验阶段、适用范围、抽样规则和检验项目。', type: 'select', value: '检验方案发布审批流程', options: qcApprovalOptions },
        { title: '检验员绑定规则', sub: '检验员必须从质检中心人员中选择；任务提交、判定、复检和放行后，检验员变更需要保留操作记录。', on: true },
        { title: '质检阶段变更审批流程', sub: '质检阶段影响方案、抽样规则和来源回写，变更时可指定审批流。', type: 'select', value: '检验方案发布审批流程', options: qcApprovalOptions },
      ] as PolicyRule[],
    },
    {
      k: 'finance',
      label: '来源与回写',
      rows: [
        { title: '来源单据回写审批流程', sub: '来源单据自动带入检验对象、批次、数量、检验阶段和推荐检验员；如手工调整来源回写规则，按指定审批流处理。', type: 'select', value: '让步放行审批流程', options: qcApprovalOptions },
        { title: '质检方案停用控制', sub: '质检方案停用后，新增检验任务不能再选择该方案；历史质检单仍可查看、复检、打印和追溯。', on: true },
      ] as PolicyRule[],
    },
    {
      k: 'approval',
      label: '审批规则',
      rows: [
        { title: '质检结果修改审批流程', sub: '质检结果已提交、已回写或已生成报告后，如果修改关键字段，按指定审批流处理。', type: 'select', value: '检验方案发布审批流程', options: qcApprovalOptions },
        { title: 'CAPA/8D关闭审批流程', sub: 'CAPA/8D验证关闭、异常作废或撤回时，必须填写原因并按指定审批流处理。', type: 'select', value: '让步放行审批流程', options: qcApprovalOptions },
      ] as PolicyRule[],
    },
  ],
});
...
const activePolicyRows = computed(() => qcPolicyConfig.value.tabs.find((tab) => tab.k === activePolicyTab.value)?.rows || qcPolicyConfig.value.tabs[0].rows);
