...
const detailActions = computed<DetailAction[]>(() => {
  if (mode.value === 'reports') return [{ key: 'export', label: '导出' }, { key: 'print', label: '打印' }, { key: 'confirm', label: '确认报表', primary: true }];
  if (mode.value === 'standards') return [{ key: 'edit', label: '修改' }, { key: 'approve', label: '提交审批' }, { key: 'print', label: '打印' }, { key: 'disable', label: '停用', danger: true }];
  if (mode.value === 'exceptions') return [
    { key: 'isolate', label: '隔离/拒收', danger: true },
    { key: 'mrb', label: 'MRB评审' },
    { key: 'recheck', label: '发起复检' },
    { key: 'release', label: '让步放行' },
    { key: 'close', label: '验证关闭', primary: true },
  ];
  return [{ key: 'execute', label: '执行质检', primary: true }];
});
    ...
    default: () => [
      ...
      h('section', { class: 'aw-form-card' }, [
        ...
        h(AwAttachmentTable, {
          ...
          onRemove: (row: { id: number | string }) => { attachRows.value = attachRows.value.filter((item) => item.id !== row.id); },
...
function editableTableSlots(rowsRef: typeof sampleRows) {
  return {
    ...
    actions: ({ row }: { row: FormLine }) => h('span', {
      ...
      onClick: () => {
        if (rowsRef.value.length > 1) rowsRef.value = rowsRef.value.filter((item) => item.id !== row.id);
      },
    }, '删除'),
  };
}
...
function handleBatchAction(actionKey: string, keys: string[]) {
  ...
  submitMessage.value = `已提交 ${keys.length || tableRows.value.length} 条记录的${label}处理。`;
}
...
function handleQuickAction(id: string, key: string) {
  ...
  if (key === 'recheck') submitQcResult(id, '返工复检').then(loadList);
}
