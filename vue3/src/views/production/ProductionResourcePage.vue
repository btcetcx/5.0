...
function confirmAction(label: string, count?: number, unit = '条数据') {
  const suffix = count ? `，将处理 ${count} ${unit}` : '';
  return window.confirm(`确认执行「${label}」吗${suffix}？`);
}

function handleBatchAction(actionKey: string, keys: string[]) {
  if (!keys.length) {
    message.value = '请先选择数据';
    return;
  }
  const label = actionKey === 'bulk' ? '批量操作' : actionKey;
  if (!confirmAction(label, keys.length)) return;
  selectedKeys.value = keys;
  message.value = `${label}已触发，选中 ${keys.length} 条`;
}
...
const ProductionDetail = defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const actions = computed<DetailAction[]>(() => detailActions(props.resource, detail.value?.status || ''));
    const auditDocument = computed<AuditDocumentSummary>(() => ({
      title: detail.value?.subject || props.config.title,
      code: detail.value?.code || '-',
      status: detail.value?.statusName || detail.value?.status || '-',
      applicant: detail.value?.ownerName || detail.value?.departmentName || '计划员',
      flowName: `${props.config.title}审批流`,
      currentNode: '生产主管审核',
    }));
    const auditApprovalNodes = computed<AuditApprovalNode[]>(() => [
      { name: '提交审批', approver: auditDocument.value.applicant || '计划员', method: '提交', state: 'done', result: '已提交', time: detail.value?.startDate || '2026-06-06 09:00' },
      { name: '生产主管审核', approver: '生产主管', method: '审批', state: 'current', result: '待审核' },
    ...
    async function handleAction(key: string) {
      ...
      if (key === 'issue-materials') {
        showOutsourceIssue.value = true;
        actionMessage.value = '已打开委外发料弹窗。';
        return;
      }
      if (key === 'dispatch') {
        router.push({ path: route.path, query: { action: '领工派工' } });
        return;
      }
      if (key === 'report') {
        router.push({ path: route.path, query: { action: '任务报工' } });
        return;
      }
      if (key === 'approve') {
        auditModalOpen.value = true;
        actionMessage.value = '请在审核弹窗中处理当前生产计划审批。';
        return;
      }
      if (['confirm', 'close', 'release-work-orders', 'receive'].includes(key)) {
        const label = actionLabel(key);
        if (!window.confirm(`确认${label} ${detail.value.subject} ${detail.value.code}？该操作会推进当前生产单据状态。`)) return;
        await runProductionAction(props.resource, detail.value.id, key as ProductionAction);
        detail.value = await getProductionDetail(props.resource, detail.value.id);
        actionMessage.value = `${label}动作已通过 mock 接口提交，并回写当前单据状态。`;
      }
    }
    ...
    async function confirmAudit(payload: AuditActionPayload) {
      ...
      if (payload.action === 'approve') {
        await runProductionAction(props.resource, detail.value.id, 'approve', { ...payload });
      } else {
        const statusName = payload.action === 'return' ? '已退回' : payload.action === 'reject' ? '已驳回' : detail.value.statusName;
        detail.value = await updateProduction(props.resource, detail.value.id, {
          status: payload.action === 'return' ? 'returned' : payload.action === 'reject' ? 'rejected' : detail.value.status,
          statusName,
          detailText: `${detail.value.detailText || ''}\n${payload.opinion || '审核弹窗已处理'}`.trim(),
        });
      }
      auditModalOpen.value = false;
      detail.value = await getProductionDetail(props.resource, detail.value.id);
      actionMessage.value = payload.action === 'approve'
        ? '审核通过已通过母版弹窗提交，并回写当前生产计划状态。'
        : `审核动作「${payload.action}」已通过母版弹窗提交。`;
    }
    ...
    async function confirmOutsourceIssue(payload: { mode: string; qty: number }) {
      if (!detail.value) return;
      if (!confirmAction(`委外${payload.mode}`, payload.qty, '数量')) return;
    ...
    function withdrawKitRow(index: number) {
      if (!confirmAction('撤回齐套预估建议')) return;
    ...
    async function confirmGeneratedPlan() {
      if (!detail.value) return;
      if (!confirmAction('生成生产计划')) return;
    ...
    async function confirmGeneratedOrders() {
      if (!detail.value) return;
      if (!confirmAction('生成生产订单')) return;
...
function detailActions(resource: ProductionResource, status: string): DetailAction[] {
  const readActions: DetailAction[] = [{ key: 'print', label: '打印' }, { key: 'export', label: '导出' }];
  const draftActions: DetailAction[] = [
    { key: 'edit', label: '编辑' },
    { key: 'delete', label: '删除', danger: true },
    ...readActions,
  ];
  const activeEditActions: DetailAction[] = [
    { key: 'edit', label: '编辑' },
    { key: 'close', label: '关闭', danger: true },
    ...readActions,
  ];

  if (resource === 'production-demands') {
    if (status === 'pendingConfirm') return [{ key: 'confirm', label: '确认需求', primary: true }];
    if (status === 'draft') return [{ key: 'confirm', label: '提交确认', primary: true }, ...draftActions];
    if (status === 'confirmed') return activeEditActions;
    return readActions;
  }
  if (resource === 'production-plans') {
    if (status === 'pendingApproval') return [{ key: 'approve', label: '审核', primary: true }];
    if (status === 'draft') return draftActions;
    if (status === 'approved') return [{ key: 'start', label: '启动计划', primary: true }, ...activeEditActions];
    if (status === 'planning') return [{ key: 'close', label: '关闭', danger: true }, ...readActions];
    return readActions;
  }
  if (resource === 'production-orders') {
    if (status === 'pendingProduction') return [{ key: 'release-work-orders', label: '释放工单', primary: true }, ...activeEditActions];
    if (status === 'draft') return draftActions;
    if (status === 'producing') return [{ key: 'close', label: '关闭', danger: true }, ...readActions];
    return readActions;
  }
  if (resource === 'production-work-orders') {
    if (status === 'pendingReport') return [{ key: 'dispatch', label: '领工派工', primary: true }, { key: 'report', label: '任务报工' }, ...readActions];
    if (status === 'producing') return [{ key: 'report', label: '任务报工', primary: true }, { key: 'close', label: '关闭', danger: true }, ...readActions];
    return readActions;
  }
  if (resource === 'outsource-orders') {
    if (status === 'pendingIssue') return [{ key: 'issue-materials', label: '委外发料', primary: true }, ...activeEditActions];
    if (status === 'processing') return [{ key: 'receive', label: '委外收货', primary: true }, { key: 'close', label: '关闭', danger: true }, ...readActions];
    return readActions;
  }
  return readActions;
}
...
const WorkOrderDispatchDetailModal = defineComponent({
  ...
  props: {
    ...
    mode: { type: String as () => 'pool' | 'assigned', default: 'pool' },
  },
  emits: ['cancel', 'claim', 'report'],
	  setup(props, { emit }) {
    return () => props.row && props.detail ? h('div', { class: 'aw-mask', onClick: () => emit('cancel') }, [
      h('div', { class: 'aw-modal lg', onClick: (event: Event) => event.stopPropagation() }, [
        ...
        h('div', { class: 'foot' }, [
          h('button', { class: 'aw-btn', type: 'button', onClick: () => emit('cancel') }, '关闭'),
          props.mode === 'pool' ? h('button', { class: 'aw-btn primary', type: 'button', onClick: () => emit('claim') }, '领工处理') : null,
          props.mode === 'assigned' ? h('button', { class: 'aw-btn primary', type: 'button', onClick: () => emit('report') }, '任务报工') : null,
        ]),
      ]),
    ]) : null;
  },
  ...
  setup(_, { emit }) {
    ...
    return () => h(AwListPage, {}, {
      default: () => [
          ...
          h(AwDataTable, {
            ...
            cell: ({ column, record, value }: { column: AwTableColumn; record: Record<string, unknown>; value: unknown }) => {
              ...
              if (column.key === 'action') {
                return h('span', { class: 'aw-link', onClick: () => { void openDispatchDetail(record as unknown as WorkOrderClaimTask | WorkOrderAssignedTask); } }, '查看');
              }
              return formatCell(value, column.numeric);
            },
          }),
        ...
        h(WorkOrderDispatchDetailModal, {
          row: detailRow.value,
          detail: dispatchDetail.value,
          mode: tab.value,
          onCancel: () => { detailRow.value = null; dispatchDetail.value = null; },
          onClaim: () => {
            if (detailRow.value) claimRow.value = detailRow.value as WorkOrderClaimTask;
          },
          onReport: () => { router.push({ path: '/production/production-work-orders', query: { action: '任务报工' } }); },
        }),
      ],
    });
  },
...
const ProductionCreate = defineComponent({
  ...
  setup(props, { emit }) {
    ...
    function confirmSource(source?: string | SourcePickerConfirmPayload) {
      ...
      if (!picked) return;
      if (lines.value.length && form.sourceCode !== picked.code && !confirmClear('当前来源和产品明细')) return;
    ...
    function removeLine(id: string) {
      ...
      if (!confirmRemove(target?.productName ? `明细「${target.productName}」` : '该明细')) return;
