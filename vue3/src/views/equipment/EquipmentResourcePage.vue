...
import type { EquipmentAsset, EquipmentModule, InspectionRecord, MaintenancePlan, MaintenanceProjectSetting, RepairOrder, SparePart, SpareRequest, SpareUsage } from '@/app/api/equipment/types';
...
const activeTree = computed(() => ({
  ...
  nodes: (moduleKey.value === 'assets'
    ? treeNodes('设备', equipmentCategoryOptions.value)
    : moduleKey.value === 'spares'
      ? treeNodes('备件', spareCategoryOptions.value)
      : currentConfig.value.tree.nodes).map((node) => ({
    ...node,
    count: node.key === 'all' ? rows.value.length : rows.value.filter((row) => countTreeNode(row, node.key)).length,
  })),
}));
const activeColumns = computed(() => currentConfig.value.columns.map((column) => {
...
const tableRows = computed(() => rows.value.filter((row) => matchView(row) && matchTree(row) && matchColumns(row)).map((row) => ({ ...row, action: '查看' })));
...
const maintenancePlansForEquipment = computed(() => rowsByType.value.maintenance.filter((item) => !form.equipmentId || item.equipmentId === form.equipmentId));
...
const configs: Record<EquipmentModule, ModuleConfig> = {
  assets: {
    ...
    columns: [
      ...
      col('model', '规格型号', 120), col('workshop', '所属车间/产线', 160), col('location', '安装位置', 120), col('assetTag', '资产标签', 130),
  ...
  spares: {
    ...
    columns: [
      ...
      col('occupiedQty', '占用库存', 90), col('warehouse', '仓库/库位', 150), col('lastUsedAt', '最近领用', 120), col('status', '预警状态', 110), actionCol(),
    ],
  },
};
...
function loadPickers() {
  return Promise.all([
    ...
    equipmentCategoryOptions.value = settingData.categories.filter((item) => item.enabled !== false).map((item) => item.name);
...
function resetForm() {
  ...
  richText.value = '';
  spareUsages.value = [];
  if (moduleKey.value === 'assets') Object.assign(form, { name: '', code: '系统自动生成', category: equipmentCategoryOptions.value[0] || '机加工设备', model: '', manufacturer: '', serialNo: '', assetTag: '', workshop: '一车间', line: '机加工产线A', location: '', ownerDept: '设备动力部', ownerName: '陈工', enabledAt: currentDate(), status: '运行中', level: '一般', ratedPower: '', maintenanceStrategy: '', inspectionStrategy: '' });
  ...
  if (moduleKey.value === 'repairs') Object.assign(form, { topic: '', equipmentId: rowsByType.value.assets[0]?.id || '', faultCategory: '机械故障', repairLevel: '一般', reporter: '陈工', reportedAt: `${currentDate()} 09:00`, impactScope: '单台设备', urgency: '中', faultDesc: '' });
...
function openDetail(id: unknown) {
  if (typeof id === 'string') router.push({ path: route.path, query: { id } });
}
...
function matchView(row: EquipmentRow) {
  ...
  if (mode === 'acceptance') return String(dict.status || '') === '待验收' || String(dict.acceptanceStatus || '') === '待验收';
  if (mode === 'exceptions') return String(dict.result || '') === '异常' || Number(dict.exceptionCount || 0) > 0 || String(dict.status || '') === '已生成维修';
...
function detailFields(row: EquipmentRow): DetailFieldItem[] {
  if (moduleKey.value === 'assets') return pickFields(row, [['设备编号', 'code'], ['设备分类', 'category'], ['规格型号', 'model'], ['制造商', 'manufacturer'], ['资产标签', 'assetTag'], ['车间/产线', 'workshop'], ['安装位置', 'location'], ['责任部门', 'ownerDept'], ['责任人', 'ownerName'], ['设备等级', 'level'], ['启用日期', 'enabledAt'], ['最近点检', 'lastInspectionAt'], ['最近保养', 'lastMaintenanceAt'], ['最近维修', 'lastRepairAt']]);
  if (moduleKey.value === 'maintenance') return pickFields(row, [['计划编号', 'code'], ['计划名称', 'name'], ['设备', 'equipmentName'], ['保养类型', 'maintenanceType'], ['周期类型', 'cycleType'], ['周期间隔', 'cycleValue'], ['计划日期', 'planDate'], ['责任人', 'ownerName'], ['保养标准', 'standard'], ['下次保养', 'nextDate']]);
  if (moduleKey.value === 'repairs') return pickFields(row, [['维修单号', 'code'], ['报修主题', 'topic'], ['设备', 'equipmentName'], ['故障分类', 'faultCategory'], ['维修等级', 'repairLevel'], ['报修人', 'reporter'], ['报修时间', 'reportedAt'], ['维修人员', 'assignee'], ['验收状态', 'acceptanceStatus']]);
  if (moduleKey.value === 'inspections') return pickFields(row, [['点检单号', 'code'], ['点检计划', 'planName'], ['设备', 'equipmentName'], ['点检类型', 'inspectionType'], ['点检人', 'inspector'], ['点检时间', 'inspectionAt'], ['点检结果', 'result'], ['异常数量', 'exceptionCount'], ['异常说明', 'exceptionDesc']]);
  return pickFields(row, [['备件编号', 'code'], ['备件名称', 'name'], ['规格型号', 'model'], ['备件分类', 'category'], ['适用设备', 'equipmentScope'], ['当前库存', 'stockQty'], ['安全库存', 'safeQty'], ['可用库存', 'availableQty'], ['占用库存', 'occupiedQty'], ['仓库/库位', 'warehouse'], ['供应商', 'supplier'], ['单价', 'unitPrice']]);
}
...
const detailActions = computed<DetailAction[]>(() => {
  if (!detailRow.value) return [];
  if (moduleKey.value === 'assets') return [{ key: 'status', label: '切换状态' }, { key: 'scrap', label: '报废', danger: true }];
  if (moduleKey.value === 'maintenance') {
  ...
  if (moduleKey.value === 'repairs') {
    ...
    if (['维修中', '已驳回'].includes(status)) actions.push({ key: 'process', label: '维修处理', primary: true });
  ...
  return [{ key: 'spareRequest', label: '生成补货记录', primary: true }];
});
...
function confirmAccept(passed: boolean) {
  ...
  acceptRepair(detailCode(detailRow.value), passed, {
    ...
    reloadDetail(passed ? '验收通过，已记录验收意见并回写设备状态' : '验收驳回，维修单回到维修中');
  });
}
...
function pickEquipment(row: OptionPickerRow) {
  ...
  if (isMaintenanceExecutionCreate.value) {
    syncMaintenancePlan(rowsByType.value.maintenance.find((item) => item.equipmentId === row.id));
  }
...
<template>
  ...
  >
    <section class="aw-form-card">
      ...
      <div class="aw-form-grid equipment-create-grid">
        <template v-if="moduleKey === 'assets'">
          ...
          <label><span>设备分类</span><select v-model="form.category"><option v-for="item in equipmentCategoryOptions" :key="item">{{ item }}</option></select></label>
          <label><span>规格型号</span><input v-model="form.model" /></label>
          <label><span>制造商</span><input v-model="form.manufacturer" /></label>
          <label><span>序列号</span><input v-model="form.serialNo" /></label>
          <label><span>资产标签</span><input v-model="form.assetTag" /></label>
          <label><span>所属车间</span><select v-model="form.workshop"><option>一车间</option><option>动力站</option><option>包装车间</option></select></label>
          <label><span>所属产线</span><select v-model="form.line"><option>机加工产线A</option><option>公共动力</option><option>包装线A</option></select></label>
          <label><span>安装位置</span><input v-model="form.location" /></label>
          ...
          <label><span>保养策略</span><select v-model="form.maintenanceStrategy"><option>月度一级保养</option><option>双周保养</option><option>待配置</option></select></label>
          <label><span>点检策略</span><select v-model="form.inspectionStrategy"><option>日点检</option><option>周点检</option><option>待配置</option></select></label>
        </template>
        ...
        <template v-else-if="moduleKey === 'repairs'">
          ...
          <label class="wide"><span>故障描述</span><textarea v-model="form.faultDesc"></textarea></label>
        </template>
        ...
        <template v-else>
          ...
          <label><span>默认仓库/库位</span><input v-model="form.warehouse" /></label>
    ...
    <section v-if="['maintenance', 'inspections'].includes(moduleKey)" class="aw-form-card">
      ...
      <aw-editable-sub-table :columns="moduleKey === 'maintenance' ? maintenanceItemColumns : inspectionItemColumns" :rows="subRows" :add-text="isMaintenanceExecutionCreate ? '补充项目' : '新增项目'" @add="openMaintenanceItemPicker">
        ...
        <template #actions="{ index }">
          <span class="aw-link" style="color:var(--aw-danger)" @click="subRows.splice(index, 1)">删除</span>
        </template>
    ...
    <section class="aw-form-card">
      <div class="aw-detail-section-title">备注</div>
      <aw-rich-text-editor v-model="richText" placeholder="请输入备注" />
    </section>
  </aw-form-page>
  ...
  <aw-detail-page v-else-if="detailRow">
    ...
    <template #header>
      <aw-detail-header :title="detailTitle(detailRow)" :code="detailCode(detailRow)" :status-text="detailStatus(detailRow)" :status-tone="toneByStatus(detailStatus(detailRow))" :metas="[{ label: '模块', value: currentConfig.title }, { label: '负责人', value: String((detailRow as any).ownerName || (detailRow as any).assignee || (detailRow as any).inspector || '-') }]" />
    </template>
    <section class="aw-card">
      ...
      <div class="equipment-tab-panel">
        ...
        <template v-else-if="activeTab.includes('基础') || activeTab.includes('计划') || activeTab.includes('报修') || activeTab.includes('点检') || activeTab.includes('备件信息')">
          <div class="aw-detail-section-title">{{ activeTab }}</div>
  ...
  <aw-list-page v-else>
    <template v-if="showListTree" #tree>
      <aw-resource-tree v-model="pickedTree" :title="activeTree.title" :total="rows.length" :nodes="activeTree.nodes" />
    </template>
    ...
    <aw-list-toolbar
      ...
      :actions="currentConfig.actions || commonToolbar"
  ...
  <aw-setting-modal :open="showRepairProcessModal" title="维修处理" width="860px" @cancel="showRepairProcessModal = false" @confirm="confirmRepairProcess">
    ...
    <div class="equipment-modal-section">
      <aw-editable-sub-table :columns="spareUsageColumns" :rows="spareUsages" add-text="添加备件" @add="addSpareUsage">
        ...
        <template #actions="{ index }"><span class="aw-link" style="color:var(--aw-danger)" @click="spareUsages.splice(index, 1)">删除</span></template>
      </aw-editable-sub-table>
    </div>
  </aw-setting-modal>

  <aw-setting-modal :open="showAcceptModal" title="维修验收" width="520px" @cancel="showAcceptModal = false" @confirm="confirmAccept(true)">
    <p>验收通过后维修单将完成，并按维修前状态回写设备状态；驳回后回到维修中。</p>
    <div class="equipment-form-grid compact">
  ...
  <aw-option-picker-modal :open="showEquipmentPicker" title="选择设备" :columns="equipmentPickerColumns" :rows="equipmentPickerRows" @cancel="showEquipmentPicker = false" @confirm="pickEquipment" />
