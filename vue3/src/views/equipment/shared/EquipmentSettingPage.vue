...
import type { ApprovalNode, PersonPickerDept, SettingTableColumn, SettingTableRow, SettingTreeItem } from '@/components/setting-page/types';
...
const numberRuleDefaults = ref({ prefix: 'EQ', separator: '-', selected: [] as string[] });
...
const approvalPersonDepts: PersonPickerDept[] = [
  {
    key: 'equipment',
    label: '设备部',
    persons: [
      { id: 'EQ001', name: '责任人', role: '设备责任人', phone: '13800006001' },
      { id: 'EQ002', name: '设备主管', role: '设备主管', phone: '13800006002' },
      { id: 'EQ003', name: '维修工程师', role: '维修工程师', phone: '13800006003' },
    ],
  },
];
...
getEquipmentSettings().then((data) => {
  ...
  numberRuleDefaults.value = { prefix: data.numberPrefix, separator: data.numberSeparator, selected: [...data.numberSelected] };
...
function saveMaintenanceItem() {
  ...
  const oldRow = rows.find((row) => row.id === maintenanceItemForm.value.id);
  if (maintenanceItemForm.value.enabled === false && oldRow?.enabled !== false && !confirmAction(`确认停用保养项目“${maintenanceItemForm.value.name}”吗？`)) return;
...
function removeMaintenanceItem(id: string | number) {
  ...
  if (index >= 0 && confirmAction(`确认删除保养项目“${settings.value.maintenanceItems[index].name}”吗？`)) settings.value.maintenanceItems.splice(index, 1);
}

function removeMaintenanceStandard(index: number) {
  const row = maintenanceItemForm.value.standards[index];
  if (!confirmAction(`确认删除保养标准“${row?.standardName || row?.itemName || index + 1}”吗？`)) return;
...
function removeCategoryRow(id: string | number) {
  ...
  if (index >= 0 && confirmAction(`确认删除${categorySettingLabel.value}“${row.name}”吗？当前关联数量：${categoryUsageCount(row.name)}。`)) rows.splice(index, 1);
}
...
function saveSettingRow() {
  ...
  const rows = settings.value[dictKey.value as keyof EquipmentSettings] as EquipmentSettingRow[];
  if (settingForm.value.enabled === false && editingRow.value?.enabled !== false && !confirmAction(`确认停用“${settingForm.value.name}”吗？`)) return;
...
function revertSwitch(event: Event | undefined, enabled: boolean) {
  const target = event?.target as HTMLInputElement | undefined;
  if (target) target.checked = enabled;
}

function toggleCategoryEnabled(row: EquipmentSettingRow, enabled: boolean, event?: Event) {
  const oldEnabled = row.enabled !== false;
  if (oldEnabled === enabled) return;
  if (!confirmAction(`确认${enabled ? '启用' : '停用'}${categorySettingLabel.value}“${row.name}”吗？`)) {
    revertSwitch(event, oldEnabled);
    return;
  }
  row.enabled = enabled;
}

function toggleMaintenanceItemEnabled(row: MaintenanceProjectSetting, enabled: boolean, event?: Event) {
  const oldEnabled = row.enabled !== false;
  if (oldEnabled === enabled) return;
  if (!confirmAction(`确认${enabled ? '启用' : '停用'}保养项目“${row.name}”吗？`)) {
    revertSwitch(event, oldEnabled);
    return;
  }
  row.enabled = enabled;
...
function setRowEnabled(row: SettingTableRow, enabled: boolean, event?: Event) {
  const source = dictRows.value.find((item) => item.id === row.id);
  if (!source) return;
  const oldEnabled = source.enabled !== false;
  if (oldEnabled === enabled) return;
  if (!confirmAction(`确认${enabled ? '启用' : '停用'}“${source.name}”吗？`)) {
    revertSwitch(event, oldEnabled);
    return;
  }
...
function removeApprovalNode(index: number) {
  if (approvalNodes.value.length <= 1) {
    window.alert('审批流程至少保留一个节点');
    return;
  }
  const node = approvalNodes.value[index];
  if (!confirmAction(`确认删除审批节点“${node?.name || index + 1}”吗？`)) return;
  approvalNodes.value.splice(index, 1);
  activeApprovalNode.value = Math.min(activeApprovalNode.value, approvalNodes.value.length - 1);
}

function removeApprovalPerson(index: number, name: string) {
  const node = approvalNodes.value[index];
  if (!node || !confirmAction(`确认从“${node.name}”中移除审批人“${name}”吗？`)) return;
  node.approvers = node.approvers.filter((item) => item !== name);
}

function resetNumberRule() {
  if (!confirmAction('确认重置设备编号规则吗？')) return;
  numberPrefix.value = numberRuleDefaults.value.prefix;
  numberSeparator.value = numberRuleDefaults.value.separator;
  numberSelected.value = [...numberRuleDefaults.value.selected];
}

function saveNumberRule() {
  window.alert('设备编号规则已保存到当前前端配置');
}
</script>
                ...
                <tr v-for="(row, index) in filteredCategoryRows" :key="row.id">
                  ...
                  <td>
                    <label class="aw-switch-line mini">
                      <input :checked="row.enabled !== false" type="checkbox" @change="toggleCategoryEnabled(row, ($event.target as HTMLInputElement).checked, $event)" />
                      <i></i>
                    </label>
                  </td>
                  <td>
            ...
            <tbody>
              <tr v-for="(row, index) in filteredMaintenanceItemRows" :key="row.id">
                ...
                <td>{{ row.standards?.length || 0 }}</td>
                  <td>
                    <label class="aw-switch-line mini">
                    <input :checked="row.enabled !== false" type="checkbox" @change="toggleMaintenanceItemEnabled(row, ($event.target as HTMLInputElement).checked, $event)" />
                    <i></i>
                  </label>
                </td>
                <td>
      ...
      <aw-setting-table :columns="dictColumns" :rows="filteredTableRows" @edit="openEditSettingRow">
        <template #cell="{ column, row }">
          <label v-if="column.key === 'enabled'" class="aw-switch-line mini">
            <input :checked="row.enabled !== false" type="checkbox" @change="setRowEnabled(row, ($event.target as HTMLInputElement).checked, $event)" />
            <i></i>
          </label>
