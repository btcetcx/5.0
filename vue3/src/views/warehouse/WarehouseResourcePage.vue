...
import type { AuditActionPayload, AuditApprovalNode, AuditDocumentSummary, DetailAction, DetailFieldItem, DetailTabItem } from '@/components/detail-page/types';
...
const configs: Record<WarehouseModule, WarehouseModuleConfig> = {
  stocks: {
    ...
    tree: {
      title: '仓库类型',
      nodes: [
        { key: 'all', label: '仓库库位', count: 3, icon: 'line-folder', level: 2, open: true },
        { key: '成品仓', label: '成品仓', count: 1, icon: 'line-warehouse', level: 3 },
        { key: '原料仓', label: '原料仓', count: 1, icon: 'line-warehouse', level: 3 },
        { key: '半成品仓', label: '半成品仓', count: 1, icon: 'line-warehouse', level: 3 },
        { key: '质检暂存仓', label: '质检暂存仓', count: 0, icon: 'line-warehouse', level: 3 },
        { key: '销售暂存仓', label: '销售暂存仓', count: 0, icon: 'line-warehouse', level: 3 },
      ],
    },
    columns: [
...
const filteredItems = computed(() => {
  ...
  return activeListRows.value.filter((item) => {
    ...
    const treeMatched = pickedTree.value === 'all'
      ...
      || item.wh === pickedTree.value
      || stockWarehouseType(item) === pickedTree.value
...
function stockWarehouseType(item: WarehouseRow) {
  if (moduleKey.value !== 'stocks') return '';
  const category = String(item.cat || '');
  if (category === '成品') return '成品仓';
  if (category === '半成品') return '半成品仓';
  if (category === '原材料') return '原料仓';
  if (category === '包装耗材') return '销售暂存仓';
  return String(item.wh || '');
}
...
function handleBatchAction(actionKey: string, keys: string[]) {
  ...
  if (!window.confirm(`确认执行「${actionLabel}」吗？将处理已选择的 ${keys.length} 条数据。`)) return;
  selectedKeys.value = keys;
...
function goList() {
  ...
  if (returnTo.startsWith('/storehouse/')) {
...
const WarehouseCreateView = defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const form = reactive<AnyRow>(createInitialWarehouseForm(module.value, props.actionMode));
    ...
    function removeAttachment(row: AttachmentRow) {
      if (attachments.value.length > 1) {
        if (!confirmRemove(row.name ? `附件「${row.name}」` : '该附件')) return;
    ...
    async function handleAction(key: string) {
      ...
      if (key === 'reset') {
        if (!confirmClear('当前仓储单据')) return;
        Object.assign(form, createInitialWarehouseForm(module.value, props.actionMode));
        rows.value = [];
        source.value = null;
        person.value = '';
      ...
      const transferFields = module.value === 'transfers' ? { transferDate: transferDate.value, transferFromWarehouse: transferFromWarehouse.value, transferToWarehouse: transferToWarehouse.value } : {};
      const res = key === 'submit'
        ? await createWarehouse(module.value, { title: title.value, ...form, source: source.value, person: person.value, warehouseLocation: warehouseLocation.value, countMode: countMode.value, countScope: countScopeValue.value, ...countDates, ...transferFields, lines: rows.value, attachments: attachments.value })
        : await updateWarehouse(module.value, 'draft', { ...form, source: source.value, person: person.value, warehouseLocation: warehouseLocation.value, countMode: countMode.value, countScope: countScopeValue.value, ...countDates, ...transferFields, lines: rows.value, attachments: attachments.value });
...
const WarehouseDetailView = defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const actions = computed<DetailAction[]>(() => detailActions(module.value, editing.value, String(detail.value?.state || '')));
    const auditDocument = computed<AuditDocumentSummary>(() => ({
      title: detail.value ? detailTitle(module.value, detail.value) : '仓储单据',
      code: String(detail.value?.code || '-'),
      status: String(detail.value?.state || '-'),
      applicant: String(detail.value?.user || detail.value?.inspector || detail.value?.manager || '仓储经办人'),
      flowName: `${warehouseModuleLabel(module.value)}审核流程`,
      currentNode: warehouseAuditNode(module.value),
    }));
    const auditApprovalNodes = computed<AuditApprovalNode[]>(() => [
      { name: '提交审批', approver: auditDocument.value.applicant || '仓储经办人', method: '提交', state: 'done', result: '已提交', time: String(detail.value?.date || '2026-06-06 09:00') },
      { name: warehouseAuditNode(module.value), approver: '仓储主管', method: '审批', state: 'current', result: '待审核' },
      { name: '仓储状态回写', approver: '系统', method: '自动', state: 'pending', result: '待回写' },
    ]);
    ...
    async function doAction(key: string) {
      ...
      if (key === 'edit') {
        ...
        feedback.value = '请在审核弹窗中处理当前仓储单据。';
      } else if (key === 'print') {
        ...
        const label = warehouseActionLabel(key);
        if (key === 'disable' && !window.confirm(`确认停用 ${detailTitle(module.value, detail.value)} 吗？`)) return;
        const nextState = warehouseActionState(module.value, key);
        await updateWarehouse(module.value, detail.value.id, { ...detail.value, ...nextState });
        detail.value = { ...detail.value, ...nextState };
        feedback.value = `${label}动作已记录，当前仓储状态已回写到演示数据。`;
      }
    }

    async function confirmAudit(payload: AuditActionPayload) {
      if (!detail.value) return;
      let nextState = payload.action === 'approve' ? warehouseActionState(module.value, 'approve') : { state: detail.value.state, tone: detail.value.tone };
      if (payload.action === 'reject') nextState = { state: '已驳回', tone: 'red' };
      if (payload.action === 'return') nextState = { state: '已退回', tone: 'yellow' };
      if (payload.action === 'approve') await approveWarehouse(module.value, detail.value.id);
      else await updateWarehouse(module.value, detail.value.id, { ...detail.value, ...nextState });
      detail.value = { ...detail.value, ...nextState };
      auditModalOpen.value = false;
      feedback.value = `审核弹窗已提交，当前仓储状态：${detail.value.state}。`;
    }
    ...
    return () => detail.value ? h(AwDetailPage, null, {
      ...
      default: () => [
        ...
        h(AwAuditActionModal, {
          ...
          title: `${warehouseModuleLabel(module.value)}审核`,
...
function createInitialWarehouseForm(module: WarehouseModule, action: string): AnyRow {
  if (module === 'inbounds') {
    return { subject: '', code: '自动生成', type: action === '直接入库' ? '直接入库' : '采购入库', department: '仓储部', sourceNo: '', operator: '当前用户' };
  }
  if (module === 'outbounds') {
    return { subject: '', code: '自动生成', type: action === '直接出库' ? '直接出库' : '销售出库', department: '仓储部', sourceNo: '', operator: '当前用户' };
  }
  if (module === 'transfers') {
    return { subject: '', code: '自动生成', reason: '库存平衡' };
  }
  if (module === 'counts') {
    return { subject: '', code: '自动生成', locked: '是' };
  }
  if (module === 'outboundQuality') {
    return { subject: '', code: '自动生成', qualityState: '待出货检验', sampling: 'AQL 0.65 外观/包装抽样13台' };
  }
  if (module === 'inboundQuality') {
    return { subject: '', code: '自动生成', qualityState: '待来料检验', sampling: 'AQL 1.0 来料抽样' };
  }
  const type = locationCreateType(action);
  if (type === '仓库') {
    return { name: '', code: 'WH2024030001', type: '成品仓', phone: '', address: '', capacity: '1200', temperature: ['常温'] };
  }
  if (type === '区域') {
    return { name: '', code: 'LS2024030001', warehouse: '仓库A', temperature: '常温', capacity: '120', volumeMode: '开启', desc: '' };
  }
  return { name: '', code: 'LS2024030001', warehouse: '仓库A', area: 'A区（常温）', capacity: '120', desc: '' };
}
...
function createBaseFields(module: WarehouseModule, action: string, person: string, source: WarehouseSource | null, warehouseLocation: string, openPerson: () => void, openSource: () => void, openWarehouse: () => void, transferDate = '', updateTransferDate: (value: string) => void = () => undefined, transferFromWarehouse = '', transferToWarehouse = '', openTransferWarehouse: (target: 'from' | 'to') => void = () => undefined, countScopeValue = '', openCountScope: () => void = () => undefined, countStartDate = '', updateCountStartDate: (value: string) => void = () => undefined, countEndDate = '', updateCountEndDate: (value: string) => void = () => undefined, form: AnyRow = {}) {
  if (module === 'inbounds') {
    const fields = [
      whField('入库主题', boundTextInput(form, 'subject', '请输入入库主题'), true),
      whField('入库单号', boundTextInput(form, 'code', '自动生成', { disabled: true })),
      whField('入库类型', boundSelect(form, 'type', ['直接入库', '采购入库', '生产入库', '销售退货入库', '委外入库'], '直接入库'), true),
      whField('入库部门', boundTextInput(form, 'department', '请选择入库部门')),
      whField('入库人员', pickerControl(person, '请选择入库人员', openPerson), true),
    ];
    if (action !== '直接入库') {
      fields.splice(3, 0, whField('关联单据', boundTextInput(form, 'sourceNo', '可手动填写关联单据')));
      fields.push(whField('经办人', boundTextInput(form, 'operator', '自动带入当前用户', { disabled: true })));
    }
    return fields;
  }
  if (module === 'outbounds') {
    const fields = [
      whField('出库主题', boundTextInput(form, 'subject', '请输入出库主题'), true),
      whField('出库单号', boundTextInput(form, 'code', '自动生成', { disabled: true })),
      whField('出库类型', boundSelect(form, 'type', ['直接出库', '内部领用', '委外领料', '销售出库', '采购退货'], '直接出库'), true),
      whField('出库部门', boundTextInput(form, 'department', '请选择出库部门')),
  ...
  if (module === 'transfers') {
    return [
      whField('调拨主题', boundTextInput(form, 'subject', '填写调拨主题'), true),
      whField('调拨单号', boundTextInput(form, 'code', '自动生成', { disabled: true })),
      ...
      whField('调拨原因', boundSelect(form, 'reason', ['请选择调拨原因', '库存平衡', '生产补料', '订单调配', '库位优化'], '库存平衡')),
    ];
  }
  if (module === 'locations') return createLocationBaseFields(action, person, openPerson, form);

  const base = [
    whField(module === 'stocks' ? '调整主题' : module === 'counts' ? '盘点主题' : module === 'outboundQuality' ? '出货检验主题' : module === 'inboundQuality' ? '来料检验主题' : module === 'locations' ? '名称' : '单据主题', boundTextInput(form, 'subject', '填写主题')),
    whField('单据编号', boundTextInput(form, 'code', '自动生成', { disabled: true })),
  ...
  if (module === 'counts') {
    base.push(
      ...
      whField('是否锁库', boundSelect(form, 'locked', ['否', '是'], '是')),
    );
  }
  if (module === 'outboundQuality') base.push(whField('OQC状态', boundSelect(form, 'qualityState', ['待出货检验', '客户验货中', '待放行审批', '已放行'], '待出货检验')), whField('抽样规则', boundTextInput(form, 'sampling', 'AQL 0.65 外观/包装抽样13台')));
  if (module === 'inboundQuality') base.push(whField('IQC状态', boundSelect(form, 'qualityState', ['待来料检验', '质检中', '让步接收', '拒收退回', '已放行'], '待来料检验')), whField('抽样规则', boundTextInput(form, 'sampling', 'AQL 1.0 来料抽样')));
  return base;
}
...
function createLocationBaseFields(action: string, person: string, openPerson: () => void, form: AnyRow = {}) {
  const type = locationCreateType(action);
  if (type === '仓库') {
    return [
      whField('仓库名称', boundTextInput(form, 'name', '请输入仓库名称'), true),
      whField('仓库编码', boundTextInput(form, 'code', '', { disabled: true })),
      whField('仓库类型', boundSelect(form, 'type', ['成品仓', '原料仓', '半成品仓', '质检暂存仓', '销售暂存仓'], '成品仓'), true),
      whField('仓库负责人', pickerControl(person, '请选择负责人', openPerson), true),
      whField('联系方式', boundTextInput(form, 'phone', '请输入联系方式')),
      whField('仓库地址', boundTextInput(form, 'address', '请输入仓库地址'), true),
      whField('仓库容量(m³)', boundTextInput(form, 'capacity', '')),
      whField('温区', boundCheckboxGroup(form, 'temperature', ['常温', '冷藏', '冻品', '恒温']), true),
    ];
  }
  if (type === '区域') {
    return [
      whField('区域名称', boundTextInput(form, 'name', '填写区域名称'), true),
      whField('区域编码', boundTextInput(form, 'code', '', { disabled: true })),
      whField('所属仓库', boundSelect(form, 'warehouse', ['仓库A', '仓库B', '仓库0545'], '仓库A'), true),
      whField('温区', boundSelect(form, 'temperature', ['常温', '冷藏', '冻品', '恒温'], '常温')),
      whField('区域容量(m³)', boundTextInput(form, 'capacity', '')),
      whField('计算存放体积', boundSelect(form, 'volumeMode', ['开启', '关闭'], '开启')),
    ];
  }
  return [
    whField('库位名称', boundTextInput(form, 'name', '填写库位名称'), true),
    whField('库位编码', boundTextInput(form, 'code', '', { disabled: true })),
    whField('所属仓库', boundSelect(form, 'warehouse', ['仓库A', '仓库B', '仓库0545'], '仓库A'), true),
    whField('所属区域', boundSelect(form, 'area', ['A区（常温）', 'B区（常温）', '冷藏区（冷藏）', '成品区（常温）'], 'A区（常温）'), true),
    whField('库位容量(m³)', boundTextInput(form, 'capacity', '')),
  ];
}
...
function boundTextInput(form: AnyRow, key: string, placeholder: string, attrs: Record<string, unknown> = {}) {
  return textInput(placeholder, {
    ...attrs,
    value: String(form[key] ?? ''),
    onInput: (event: Event) => {
      form[key] = (event.target as HTMLInputElement).value;
      const original = attrs.onInput;
...
function boundSelect(form: AnyRow, key: string, options: string[], fallback = '') {
  const value = String(form[key] ?? fallback);
  if (form[key] === undefined) form[key] = value;
  return h('select', {
    class: 'aw-select',
    value,
    onChange: (event: Event) => {
      form[key] = (event.target as HTMLSelectElement).value;
    },
  }, options.map((item) => h('option', { key: item, value: item.startsWith('请选择') ? '' : item }, item)));
}
...
function boundCheckboxGroup(form: AnyRow, key: string, options: string[]) {
  const values = new Set(Array.isArray(form[key]) ? form[key] as string[] : []);
  return h('div', { class: 'warehouse-check-options' }, options.map((item) => h('span', { key: item, class: 'warehouse-check-option' }, [
    h('input', {
      type: 'checkbox',
      checked: values.has(item),
      onChange: (event: Event) => {
        const next = new Set(Array.isArray(form[key]) ? form[key] as string[] : []);
        if ((event.target as HTMLInputElement).checked) next.add(item);
        else next.delete(item);
        form[key] = Array.from(next);
      },
    }),
    h('em', item),
  ])));
...
function removeCreateRow(rows: { value: AnyRow[] }, index: number) {
  if (confirmRemove('该明细')) rows.value.splice(index, 1);
}
...
function detailActions(module: WarehouseModule, editing: boolean, state = ''): DetailAction[] {
  ...
  if (needsWarehouseAudit(module, state)) return [{ key: 'approve', label: '审核', primary: true }];
  if (module === 'stocks') return [{ key: 'edit', label: '编辑' }, ...readActions];
  if (module === 'locations') {
    if (['禁用', '维护'].includes(state)) return [{ key: 'enable', label: '启用', primary: true }, ...readActions];
    return [{ key: 'edit', label: '编辑' }, { key: 'disable', label: '停用', danger: true }, ...readActions];
  }
  if (module === 'inbounds') {
    if (state === '质检中') return [{ key: 'submit-result', label: '质检确认', primary: true }, ...readActions];
    if (state === '待上架') return [{ key: 'shelf', label: '上架确认', primary: true }, ...readActions];
    return readActions;
  }
  if (module === 'outbounds') {
    if (state === '已审核待占用') return [{ key: 'occupy', label: '占用确认', primary: true }, ...readActions];
    if (state === '拣货中') return [{ key: 'pick-review', label: '复核确认', primary: true }, ...readActions];
    return readActions;
  }
  if (module === 'transfers') {
    if (state === '调拨出库待确认') return [{ key: 'transfer-out', label: '调出确认', primary: true }, ...readActions];
    if (state === '调入确认中') return [{ key: 'transfer-in', label: '调入确认', primary: true }, ...readActions];
    return readActions;
  }
  if (module === 'counts') {
    if (state === '未开始') return [{ key: 'start-count', label: '开始盘点', primary: true }, ...readActions];
    if (state === '盘点中') return [{ key: 'submit-recheck', label: '提交复盘', primary: true }, ...readActions];
    if (state === '复盘中') return [{ key: 'confirm-diff', label: '差异确认', primary: true }, ...readActions];
    return readActions;
  }
  if (module === 'outboundQuality' || module === 'inboundQuality') {
    if (['待出货检验', '待来料检验'].includes(state)) return [{ key: 'start-inspection', label: '开始检验', primary: true }, ...readActions];
    if (['客户验货中', '质检中'].includes(state)) return [{ key: 'submit-result', label: '提交结果', primary: true }, ...readActions];
    return readActions;
  }
  return readActions;
}

function needsWarehouseAudit(module: WarehouseModule, state: string) {
  if (module === 'outbounds') return ['OQC待放行', '待复核'].includes(state);
  if (module === 'counts') return state === '差异待调整';
  if (module === 'outboundQuality' || module === 'inboundQuality') return state === '待放行审批';
  return state === '已收货待检';
}

function warehouseModuleLabel(module: WarehouseModule) {
  const labels: Partial<Record<WarehouseModule, string>> = {
    inbounds: '入库单',
    outbounds: '出库单',
    transfers: '调拨单',
    counts: '盘点单',
    inboundQuality: '来料检验',
    outboundQuality: '出货检验',
    stocks: '库存',
    locations: '库位',
  };
  return labels[module] || '仓储单据';
}

function warehouseAuditNode(module: WarehouseModule) {
  if (module === 'counts') return '盘点差异审核';
  if (module === 'inboundQuality') return 'IQC放行审核';
  if (module === 'outboundQuality') return 'OQC放行审核';
  if (module === 'outbounds') return '出库复核审核';
  return '仓储主管审核';
}

function warehouseActionLabel(key: string) {
...
function warehouseActionState(module: WarehouseModule, key: string): Partial<WarehouseDetail> {
  if (key === 'approve') {
    if (module === 'counts') return { state: '已调整', tone: 'green' };
    if (module === 'outboundQuality' || module === 'inboundQuality') return { state: '已放行', tone: 'green' };
    if (module === 'outbounds') return { state: '拣货中', tone: 'blue' };
    return { state: '待上架', tone: 'blue' };
  }
  const map: Record<string, Partial<WarehouseDetail>> = {
    shelf: { state: '已上架过账', tone: 'green' },
    occupy: { state: '拣货中', tone: 'blue' },
    'pick-review': { state: '已过账', tone: 'green' },
    'transfer-out': { state: '调入确认中', tone: 'blue' },
    'transfer-in': { state: '已完成', tone: 'green' },
    'start-count': { state: '盘点中', tone: 'blue' },
    'submit-recheck': { state: '复盘中', tone: 'yellow' },
    'confirm-diff': { state: '差异待调整', tone: 'yellow' },
    'start-inspection': { state: module === 'inboundQuality' ? '质检中' : '客户验货中', tone: 'blue' },
    'submit-result': { state: '已放行', tone: 'green' },
    enable: { state: module === 'locations' ? '可用' : '启用', tone: 'green' },
    disable: { state: module === 'locations' ? '禁用' : '已停用', tone: 'gray' },
  };
  return map[key] || {};
}
...
function renderDetailTab(module: WarehouseModule, detail: WarehouseDetail, tab: string, editing: boolean) {
  ...
  if (module === 'stocks' && tab === '占用冻结') return renderSimpleTable(['仓库', '区域', '库位', '库位编号', '批次', '库存数量', '冻结数量', '占用数量', '可用数量', '在途数量', '状态'], detail.lines || []);
  if (module === 'stocks' && tab === '出库记录') return renderSimpleTable(['来源单据', '来源明细', '业务类型', '批次', '库位', '变动数', '变动后', '状态'], stockFlowByDirection(detail, '出库'));
  if (module === 'stocks' && tab === '入库记录') return renderSimpleTable(['来源单据', '来源明细', '业务类型', '批次', '库位', '变动数', '变动后', '状态'], stockFlowByDirection(detail, '入库'));
  if (isQualityModule(module) && tab === '来源记录') return renderSimpleTable(['来源单据', '检验对象', '批次/样本', '质检方案', '抽样规则', '状态'], qualitySourceRows(detail));
  if (isQualityModule(module) && tab === '抽样记录') return renderSimpleTable(['样本号', '物品编码', '批次', '抽样数量', '抽样规则', '检验状态'], qualitySampleRows(detail));
  if (tab === '检验明细') return renderSimpleTable(['检验项目名称', '检验方法', '检验值类型', '标准值', '上限', '下限', '样本号/实测值', '缺陷等级', '检验结论'], detail.lines || []);
  if (isQualityModule(module) && tab === '不良处置') return renderSimpleTable(['检验项目', '缺陷等级', '检验结论', '处置建议', '责任归属', '状态'], qualityDefectRows(detail));
  if (isQualityModule(module) && tab === '复检记录') return renderSimpleTable(['复检单号', '复检对象', '复检原因', '复检人', '复检结论', '状态'], qualityRecheckRows(detail));
  if (isQualityModule(module) && tab === '放行/拒收记录') return renderSimpleTable(['处理单号', '处理类型', '来源单据', '处理数量', '审批人', '处理结果'], qualityReleaseRows(detail));
...
function stockFlowByDirection(detail: WarehouseDetail, direction: string) {
  const rows = (detail.records || []).filter((row) => String(row.direction || '') === direction);
  return rows.length ? rows : detail.records || [];
}

function qualitySourceRows(detail: WarehouseDetail) {
  return [{
    id: 'quality-source',
    来源单据: detail.source,
    检验对象: detail.object,
    '批次/样本': detail.qty,
    质检方案: detail.plan,
    抽样规则: detail.sampling,
    状态: detail.state,
  }];
}

function qualitySampleRows(detail: WarehouseDetail) {
  return (detail.lines || []).map((row, index) => ({
    id: `quality-sample-${row.id || index}`,
    样本号: `${String(detail.lot || detail.code || 'SAMPLE')}-${index + 1}`,
    物品编码: row.productCode || detail.object,
    批次: row.batch || detail.lot,
    抽样数量: detail.qty,
    抽样规则: detail.sampling,
    检验状态: row.result || detail.state,
  }));
}

function qualityDefectRows(detail: WarehouseDetail) {
  const rows = (detail.lines || []).filter((row) => !['合格', '通过'].includes(String(row.result || '')));
  const sourceRows = rows.length ? rows : detail.lines || [];
  return sourceRows.map((row, index) => ({
    id: `quality-defect-${row.id || index}`,
    检验项目: row.item,
    缺陷等级: row.defect,
    检验结论: row.result,
    处置建议: String(row.result || '') === '合格' ? '无需处置' : '隔离复检/让步审批',
    责任归属: '待判定',
    状态: String(row.result || '') === '合格' ? '已关闭' : '待处理',
  }));
}

function qualityRecheckRows(detail: WarehouseDetail) {
  return [{
    id: 'quality-recheck',
    复检单号: `${String(detail.code || 'QC')}-RC`,
    复检对象: detail.object,
    复检原因: String(detail.state || '').includes('拒收') ? '客户/供应商拒收复检' : '抽样异常复核',
    复检人: detail.inspector || '质量主管',
    复检结论: String(detail.state || '').includes('拒收') ? '待复检' : '无需复检',
    状态: String(detail.state || '').includes('拒收') ? '待处理' : '已关闭',
  }];
}

function qualityReleaseRows(detail: WarehouseDetail) {
  const isRejected = String(detail.state || '').includes('拒收') || String(detail.state || '').includes('拦截');
  return [{
    id: 'quality-release',
    处理单号: `${String(detail.code || 'QC')}-${isRejected ? 'RJ' : 'RL'}`,
    处理类型: isRejected ? '拒收/拦截' : '放行/让步',
    来源单据: detail.source,
    处理数量: detail.qty,
    审批人: '质量主管',
    处理结果: isRejected ? '待退回或重检' : '允许后续入库/出库',
  }];
}
...
function normalizedRows(headers: string[], rows: AnyRow[]) {
  ...
  return rows.map((row) => {
    ...
    return Object.fromEntries(headers.map((label, index) => [label, row[label] ?? detailHeaderValue(label, row) ?? row[keys[index]] ?? '-']));
  });
}

const detailHeaderKeyMap: Record<string, string[]> = {
  操作时间: ['time'],
  操作人: ['user'],
  操作类型: ['action'],
  说明: ['note', 'remark'],
  来源单据: ['sourceDoc', 'source', 'sourceNo'],
  来源明细: ['sourceLine'],
  业务类型: ['businessType', 'type'],
  方向: ['direction'],
  批次: ['batch'],
  库位: ['location'],
  质量状态: ['qualityState', 'quality'],
  成本层: ['costLayer'],
  单位成本: ['unitCost'],
  变动前: ['beforeQty'],
  变动数: ['changeQty'],
  ...
  检验结论: ['result'],
  调整单号: ['adjustNo'],
  差异类型: ['result', 'countResult'],
  差异数量: ['diffQty'],
  处理方式: ['dispose'],
  审批状态: ['adjustStatus'],
  过账结果: ['postStatus', 'status'],
  库存释放: ['releaseState'],
  区域编号: ['code'],
  区域名称: ['name'],
  所属仓库: ['warehouse'],
  ...
  库位数: ['locationCount'],
  区域描述: ['desc'],
  库位编号: ['code', 'locationCode'],
  库位名称: ['name', 'location'],
  所属区域: ['area'],
  库位描述: ['desc'],
  负责人: ['manager'],
  物品编码: ['itemCode', 'code', 'productCode'],
  物品名称: ['itemName', 'name', 'productName'],
  规格型号: ['model'],
  类型: ['type'],
  单位: ['unit'],
  批次号: ['batch'],
...
function detailHeaderValue(label: string, row: AnyRow) {
  const keys = detailHeaderKeyMap[label] || [];
