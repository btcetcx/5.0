...
const attachmentRows = ref<AttachmentRow[]>(initialAttachmentRows());
...
function confirmSource(payload: SourcePickerConfirmPayload) {
  ...
  const willReplaceSource = !!source.value && source.value.code !== picked.code;
  const willReplaceCustomer = !!customer.name && customer.name !== picked.customer;
  if ((willReplaceSource || willReplaceCustomer) && hasSourceDraft() && !confirmClear('当前来源、产品选择和问题证据')) return;
  source.value = picked;
...
function confirmCustomer() {
  ...
  if (customer.name && customer.name !== picked.name && hasSourceDraft() && !confirmClear('当前客户下的来源、产品和问题说明')) return;
...
async function submitAfterSales() {
  ...
  try {
    ...
    formMessage.value = error instanceof Error ? error.message : '售后提交失败，请检查当前草稿后重试。';
  }
}

function initialAttachmentRows(): AttachmentRow[] {
  return [
...
function clearProductEdits() {
  Object.keys(productEdits).forEach((key) => delete productEdits[key]);
}

function hasSourceDraft() {
  return Boolean(source.value || subject.value.trim() || selectedProductDetails.value.length || description.value.trim());
}
...
function resetDraft() {
  ...
  source.value = null;
  subject.value = '';
  selectedType.value = '维修处理';
  selectedHandlingMethod.value = '维修';
  description.value = '';
  showSourcePicker.value = false;
  showCustomerPicker.value = false;
  ...
  customer.manager = '';
  customer.group = '';
  formMessage.value = '当前售后草稿已重置。';
}

function confirmRemove(label: string) {
  return window.confirm(`确认删除${label}吗？删除后当前附件清单会立即移除。`);
}
