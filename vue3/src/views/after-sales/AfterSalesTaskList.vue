<template>
  <aw-detail-page v-if="currentView === 'detail' && activeTask">
    ...
    <section v-if="activeDetailTab === 'current'" class="aw-form-card">
      ...
      <div v-if="taskMessage" class="aw-form-note as-task-message">{{ taskMessage }}</div>
...
async function advanceTask() {
  if (!activeTask.value) return;
  const docLabel = `${activeTask.value.linkedDocumentCode}（${activeTask.value.linkedDocumentStatus}）`;
  if (!window.confirm(`确认推进派生单据 ${docLabel} 吗？该操作会回写关联售后单状态。`)) return;
  try {
    await advanceAfterSalesTask(activeTask.value.id);
    const taskId = activeTask.value.id;
    await loadData();
    await loadDetail(taskId);
    taskMessage.value = activeTask.value
      ? `派生单据已推进到「${activeTask.value.linkedDocumentStatus}」，任务状态为「${activeTask.value.status}」。`
      : '派生单据已推进，任务状态已更新。';
  } catch (error) {
    taskMessage.value = error instanceof Error ? error.message : '任务推进失败，请稍后重试。';
  }
}
