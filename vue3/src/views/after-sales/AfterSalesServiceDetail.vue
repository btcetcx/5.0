...
import { addAfterSalesCommunicationLog, afterSalesMethodDocumentMap, advanceAfterSalesLinkedDocument, closeAfterSalesService, getAfterSalesRequiredDocumentTypes, getAfterSalesService, listAfterSalesLinkedDocuments, reviewAfterSalesService } from '@/app/api/after-sales/resources';
...
import type { AfterSalesOpinionPayload, AuditActionPayload } from '@/components/detail-page/types';
...
const detailActions = computed(() => {
  ...
  if (entryFromMineTask.value) {
    actions.push({ key: 'opinion', label: '处理意见', primary: true });
    return actions;
  }
  if (service.value?.status === 'pendingReview') {
    actions.push({ key: 'audit', label: '单据审核', primary: true });
    return actions;
  }
  return [
    { key: 'edit', label: '修改' },
...
async function handleAction(key: string) {
  ...
  if (key === 'close' && service.value.status === 'pendingCloseConfirm') {
...
function confirmOpinionAction(_payload: AfterSalesOpinionPayload) {
