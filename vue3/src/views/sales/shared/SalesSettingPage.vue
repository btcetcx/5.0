...
const strategyTabs = ref(template.value.strategies.tabs.map((tab) => ({ ...tab, rows: cloneStrategyRows(tab.rows) })));
...
const strategyTabsWithApprovals = computed(() => strategyTabs.value.map((tab) => ({
  ...tab,
  rows: withApprovalOptions(tab.rows),
})));
...
function withApprovalOptions(rows: StrategyRule[]): StrategyRule[] {
  return rows.map((row) => {
    const next = row.type === 'select' && row.key === 'needApproval'
      ? { ...row, options: approvalOptions.value, value: row.value || approvalOptions.value[0] }
      : { ...row };
    if (row.children) next.children = withApprovalOptions(row.children);
    return next;
  });
}
...
function updateStrategyRows(rows: StrategyRule[], rule: StrategyRule): StrategyRule[] {
  return rows.map((row) => {
    if (row.key === rule.key) return { ...rule, children: rule.children ? cloneStrategyRows(rule.children) : undefined };
