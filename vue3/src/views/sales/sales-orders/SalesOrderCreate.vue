  ...
  <aw-form-page :actions="formActions" @back="emit('back')" @action="handleAction">
    <section class="aw-form-card">
      ...
      <div class="aw-form-grid">
        ...
        <div class="aw-field">
          <label>出库方式 <span class="aw-help-dot" data-tip="自动出库：满足库存、信用、审批等条件后，可按规则自动进入出库流程；手动出库：订单确认后需发起出库申请，再进入出库流程。">?</span></label>
          <select v-model="form.deliveryMode" class="aw-select"><option>自动出库</option><option>手动出库</option></select>
        </div>
      </div>
    </section>
