  ...
  <aw-form-page :actions="formActions" @back="router.push('/sales/customers')" @action="handleFormAction">
    <section class="aw-form-card">
      ...
      <div class="aw-form-grid">
        ...
        <div class="aw-field">
          <label>客户结算币种</label>
          <select v-model="form.currency" class="aw-select">
            <option v-for="item in currencyOptions" :key="item.value" :value="item.value">{{ item.label }}</option>
          </select>
        </div>
      </div>
...
function resetForm() {
  form.value = { name: '', groupName: '', pinyinCode: '', creditCode: '', level: '', industry: '', freightPay: '客户支付', currency: 'CNY' };
...
function removeContact(id: number) { if (contactRows.value.length > 1 && confirmRemove('联系人')) contactRows.value = contactRows.value.filter((row) => row.id !== id); }
...
function removeFinance(id: number) { if (financeRows.value.length > 1 && confirmRemove('财务信息')) financeRows.value = financeRows.value.filter((row) => row.id !== id); }
function addAddress() { addressRows.value.push({ id: Date.now(), addressType: '收货地址' }); }
function removeAddress(id: number) { if (addressRows.value.length > 1 && confirmRemove('地址信息')) addressRows.value = addressRows.value.filter((row) => row.id !== id); }
...
function removeAttach(row: AttachmentRow) { if (attachRows.value.length > 1 && confirmRemove(`附件 ${row.name || ''}`)) attachRows.value = attachRows.value.filter((item) => item.id !== row.id); }
