  ...
  <aw-form-page :actions="formActions" @back="emit('back')" @action="handleFormAction">
    <section class="aw-form-card">
      ...
      <div class="aw-form-grid">
        <div class="aw-field"><label class="req">报价主题</label><input v-model="form.topic" class="aw-input" placeholder="请输入内容" /></div>
        ...
        <div class="aw-field"><label class="req">开始日期</label><input v-model="form.quoteDate" class="aw-input" type="date" /></div>
    ...
    <section class="aw-form-card">
      ...
      <aw-editable-sub-table :columns="productColumns" :rows="products" add-text="新增明细" :action-width="140" @add="showProductPicker = true">
        <template #cell="{ column, row }">
          ...
          <input v-else-if="column.key === 'price'" v-model="row.price" class="aw-input" placeholder="¥ 0:00" />
        </template>
            ...
            <table class="aw-table">
              ...
              <tbody>
                <tr v-for="row in filteredProductPickerRows" :key="row.productNo" @click="togglePickedProduct(row)">
                  ...
                  <td class="aw-num">{{ row.productNo }}</td><td>{{ row.productName }}</td><td>{{ row.model }}</td><td>{{ row.category }}</td><td>{{ row.unit }}</td><td class="aw-num">{{ money(row.price) }}</td>
                </tr>
              </tbody>
            </table>
          </div>
      ...
      <div class="aw-modal lg" @click.stop>
        ...
        <div class="body">
          <div class="aw-form-grid" style="margin-bottom:14px">
            ...
            <div>基础单价：{{ tierProduct.price ? money(tierProduct.price) : '-' }}</div>
          </div>
          <table class="aw-table">
...
const quoteAmountTotal = computed(() => money(products.value.reduce((sum, row) => sum + parseMoneyValue(row.price), 0)));
...
function removeProduct(id: string) {
  if (confirmRemove('报价产品')) products.value = products.value.filter((row) => row.id !== id);
}
...
function removeTier(id: string) {
  if (confirmRemove('阶梯报价')) tiers.value = tiers.value.filter((row) => row.id !== id);
}
...
async function handleFormAction(key: string) {
  if (key === 'reset') {
    if (!confirmClear('当前报价草稿')) return;
    form.value = { topic: '', quoteDate: '', expireDate: '' };
    quoteCategory.value = '';
    scopeText.value = '';
    products.value = [];
