...
export interface PrdFlowDiagramNode {
  id: string;
  title: string;
  summary: string;
  badge?: string;
...
export interface PrdFlowDiagramLane {
  title: string;
  description?: string;
  nodes: PrdFlowDiagramNode[];
}

export interface PrdFlowDiagramEdge {
  from: string;
  to: string;
  label: string;
  tone?: 'main' | 'support' | 'loop';
}

export interface PrdFlowDiagram {
  lanes: PrdFlowDiagramLane[];
  edges: PrdFlowDiagramEdge[];
}

export type PrdSectionId = 'details' | 'fields' | 'states' | 'flows' | 'acceptance';
...
export interface PrdDocument {
  ...
  writeBacks: string[];
  acceptance: string[];
  sectionIds?: PrdSectionId[];
  flowDiagram?: PrdFlowDiagram;
}
...
] satisfies Array<{ id: PrdSectionId; label: string }>;
...
const receivableStates: PrdState[] = [
  ...
  { name: '已调整', meaning: '因售后、折让、退货或调整单改变应收金额。', trigger: '应收调整单审核通过。', next: '待收款 / 已核销 / 已关闭' },
  { name: '已关闭', meaning: '应收业务不再允许新增收款、调整或红冲外动作。', trigger: '余额为零且所有派生动作完成。', next: '终态' },
  { name: '开票状态：待开票', meaning: '应收来源尚未完成销项开票。', trigger: '应收记录生成后等待开票。', next: '已开票 / 待红冲' },
  { name: '开票状态：已开票', meaning: '销项发票已开具并回写应收。', trigger: '财务完成开票。', next: '待红冲' },
  { name: '开票状态：待红冲', meaning: '售后退货、退款或调整触发红冲处理。', trigger: '售后调整或红冲申请进入应收。', next: '已调整 / 已关闭' },
  { name: '核销状态：待核销', meaning: '应收尚未匹配收款流水。', trigger: '应收生成或收款未匹配。', next: '部分核销 / 已核销' },
  { name: '核销状态：部分核销', meaning: '已有部分金额完成核销。', trigger: '收款流水部分匹配应收。', next: '已核销' },
  { name: '核销状态：已核销', meaning: '应收金额已完成核销。', trigger: '收款流水全部匹配应收。', next: '已核销 / 已关闭' },
];
...
const payableStates: PrdState[] = [
  ...
  { name: '到票状态：待到票', meaning: '供应商发票尚未到票。', trigger: '暂估应付生成或付款条件要求到票。', next: '已到票 / 已认证' },
  { name: '到票状态：已到票', meaning: '供应商发票已收到但未完成认证。', trigger: '财务登记到票。', next: '已认证' },
  { name: '到票状态：已认证', meaning: '进项发票已完成认证。', trigger: '税务认证通过。', next: '待付款 / 已付款' },
  { name: '到票状态：待红冲', meaning: '采购退货或应付冲减等待供应商发票红冲处理。', trigger: '采购退货、应付冲减或红冲申请进入应付。', next: '已冲减 / 已关闭' },
  { name: '三单匹配状态：待匹配', meaning: '采购订单、入库记录和进项发票尚未完成匹配。', trigger: '应付暂估、到票或入库完成。', next: '部分匹配 / 已匹配' },
  { name: '三单匹配状态：部分匹配', meaning: '三单已有部分来源匹配，仍有数量或金额缺口。', trigger: '订单、入库、发票中至少两方匹配。', next: '已匹配' },
  { name: '三单匹配状态：已匹配', meaning: '采购订单、入库记录和进项发票完成匹配。', trigger: '三单来源、数量和金额匹配通过。', next: '待付款 / 已付款' },
  { name: '核销状态：待核销', meaning: '付款流水尚未匹配应付。', trigger: '应付生成或付款未匹配。', next: '部分核销 / 已核销' },
  { name: '核销状态：部分核销', meaning: '已有部分付款金额完成核销。', trigger: '付款流水部分匹配应付。', next: '已核销' },
  { name: '核销状态：已核销', meaning: '应付金额已完成核销。', trigger: '付款流水全部匹配应付。', next: '已核销 / 已关闭' },
];
...
const invoiceStates: PrdState[] = [
  ...
  { name: '勾稽/认证状态：待勾稽', meaning: '销项发票尚未与来源财务对象建立勾稽关系。', trigger: '发票创建或导入后等待勾稽。', next: '已认证 / 已红冲 / 异常' },
  { name: '勾稽/认证状态：待收票', meaning: '进项发票尚未收到，等待供应商发票。', trigger: '采购来源生成收票待办。', next: '待认证 / 异常' },
  { name: '勾稽/认证状态：待认证', meaning: '进项发票等待认证。', trigger: '收票登记完成。', next: '已认证 / 异常' },
  { name: '勾稽/认证状态：已认证', meaning: '发票认证或勾稽已完成。', trigger: '税务认证或勾稽处理通过。', next: '已认证 / 已红冲' },
  { name: '勾稽/认证状态：待红冲', meaning: '红冲发票等待处理。', trigger: '售后退货、退款或采购退货触发红冲。', next: '已红冲 / 异常' },
  { name: '勾稽/认证状态：已红冲', meaning: '发票已完成红冲处理。', trigger: '红冲发票处理完成。', next: '终态' },
  { name: '勾稽/认证状态：异常', meaning: '发票勾稽、认证或红冲处理存在异常。', trigger: '金额、来源、税额、抬头或接口校验不通过。', next: '待勾稽 / 待认证 / 待红冲' },
];

const settlementStates: PrdState[] = [
  { name: '对账状态：待对账', meaning: '流水等待银行流水匹配。', trigger: '流水进入收付核销列表。', next: '已对账 / 差异待处理' },
  { name: '对账状态：已对账', meaning: '银行流水与系统流水金额、日期、账户匹配。', trigger: '银行对账匹配通过。', next: '核销状态：待核销 / 核销状态：已核销' },
  { name: '对账状态：差异待处理', meaning: '银行流水与系统流水存在金额、账户、日期或摘要差异。', trigger: '银行对账匹配失败。', next: '待对账 / 已对账' },
  { name: '核销状态：待核销', meaning: '流水尚未匹配到应收、应付或退款对象。', trigger: '流水进入核销处理。', next: '部分核销 / 已核销' },
  { name: '核销状态：部分核销', meaning: '流水仅匹配部分财务对象或部分金额。', trigger: '核销部分完成。', next: '已核销' },
  { name: '核销状态：已核销', meaning: '流水金额全部分配到应收、应付或退款单。', trigger: '核销处理通过。', next: '终态' },
];
...
const commonCreateFields: PrdField[] = [
  { name: '来源单据', source: '来源选择弹窗', description: '选择销售、采购、售后、仓储或财务对象作为创建依据。', rules: '正常业务必须选择来源；无来源必须填写补录原因。' },
  { name: '往来单位', source: '来源单据带入', description: '客户、供应商或售后客户。', rules: '选择来源后自动带入，可编辑时必须保留审计记录。' },
  { name: '来源类型', source: 'sourceRef.sourceType', description: '标识来源对象类型。', rules: '使用契约枚举，例如 sales_order、purchase_order、after_sales、supplement。' },
  { name: '来源引用', source: 'sourceRef', description: '来源类型和来源单号组合展示。', rules: '必须可点击回到业务来源；无来源记录需要显示补录原因。' },
  ...
  { name: '备注/说明', source: '人工填写', description: '差异原因、核销说明、调整说明。', rules: '补录单据、差异处理、红冲处理必填。' },
];

const dashboardFields: PrdField[] = [
  { name: '待收款应收', source: '财务工作台 kpis', description: '统计未收金额大于 0 且未关闭的应收单。', rules: '必须作为待办事项卡片展示，点击进入应收列表并带待收款筛选。' },
  { name: '逾期应收', source: '财务工作台 kpis', description: '到期日早于当前业务日期且未收金额大于 0 的应收。', rules: '必须作为待办事项卡片展示，点击进入应收列表并带逾期筛选。' },
  { name: '待付款应付', source: '财务工作台 kpis', description: '统计可付款金额大于 0 且等待付款处理的应付。', rules: '必须作为待办事项卡片展示，点击进入应付列表或付款申请入口。' },
  { name: '待开票', source: '财务工作台 kpis', description: '统计销售、售后或补录来源中等待销项开票的记录。', rules: '必须作为待办事项卡片展示，点击进入发票管理并带待开票筛选。' },
  { name: '待收票认证', source: '财务工作台 kpis', description: '统计采购进项发票中等待收票、勾稽或认证的记录。', rules: '必须作为待办事项卡片展示，点击进入发票管理并带待认证筛选。' },
  { name: '待退款付款', source: '财务工作台 kpis', description: '统计售后退款、采购退货退款或其他退款付款待处理记录。', rules: '必须作为待办事项卡片展示，点击进入收付核销退款付款入口。' },
  { name: '待应收调整', source: '财务工作台 kpis', description: '统计售后、红冲、折让或调整单产生的应收调整待办。', rules: '必须作为待办事项卡片展示，点击进入应收调整处理入口。' },
  { name: '待三单匹配', source: '财务工作台 kpis', description: '统计采购订单、采购入库、进项发票等待三单匹配或差异确认的记录。', rules: '必须作为待办事项卡片展示，点击进入应付三单匹配入口。' },
  { name: '待银行对账', source: '财务工作台 kpis', description: '统计银行流水未对账、系统流水未匹配或差异待处理记录。', rules: '必须作为待办事项卡片展示，点击进入收付核销银行对账页面。' },
];

const receivableFields: PrdField[] = [
  { name: '应收单号', source: 'Receivable.code', description: '应收单编号。', rules: '自动生成，列表和详情主链接。' },
  { name: '来源类型', source: 'sourceRef.sourceType', description: '销售订单、销售合同、售后单或无来源补录。', rules: '不得把仓储确认点当作主来源；仓储出库只能作为确认节点。' },
  { name: '来源单据', source: 'sourceRef.sourceCode', description: '生成应收的业务单据。', rules: '必须可点击跳回销售、售后或补录来源说明。' },
  ...
  { name: '核销状态/状态', source: '收付核销', description: '待核销、部分核销、已核销及应收业务状态。', rules: '由收款和核销推进，不允许越过终态条件。' },
];

const payableFields: PrdField[] = [
  { name: '应付单号', source: 'Payable.code', description: '应付单编号。', rules: '暂估、到票后应付、冲减应付统一编号。' },
  ...
  { name: '到票状态', source: '发票管理', description: '待到票、已到票、已认证、待红冲。', rules: '未到票付款需命中策略例外；采购退货或冲减场景可进入待红冲。' },
  { name: '三单匹配状态', source: '订单/入库/发票匹配结果', description: '待匹配、部分匹配、已匹配。', rules: '金额差异超阈值必须进入差异审批。' },
  { name: '核销状态/状态', source: '收付核销', description: '付款和核销状态。', rules: '付款登记不等于核销完成。' },
];
...
const invoiceFields: PrdField[] = [
  ...
  { name: '勾稽/认证状态', source: '税务认证/发票勾稽', description: '待勾稽、待收票、待认证、已认证、待红冲、已红冲、异常。', rules: '认证通过后回写采购到票和三单匹配；红冲和异常按当前发票方向展示。' },
];

const settlementFields: PrdField[] = [
  { name: '流水单号', source: 'Settlement.code', description: '资金流水编号。', rules: '银行导入和补录登记都必须保留编号。' },
  ...
  { name: '金额/账户', source: '银行流水或补录登记', description: '到账或付款金额及账户。', rules: '账户必须在资金策略中启用。' },
...
const settingFields: PrdField[] = [
  ...
  { name: '公共母版', source: '设置中心', description: '自定义字段、编号、审批、打印模板。', rules: '复用客户管理和销售设置的公共交互口径。' },
];

const receivableSourceDetailFields: PrdField[] = [
  { name: '来源主题', source: 'FinanceResourcePage.modules.receivables.tabs.source / recordRows(row, source)', description: '应收来源明细区展示来源业务主题。', rules: '来自销售订单、销售合同或售后应收调整节点。' },
  { name: '业务类型', source: 'recordColumns(source)', description: '区分销售订单、销售合同、售后单等来源类型。', rules: '不得用仓储出库替代主来源，仓储只作为确认节点。' },
  { name: '出库/签收确认记录', source: 'recordRows(row, source)', description: '展示销售出库、签收或履约确认记录。', rules: '只用于确认应收生成条件，不修改来源明细。' },
  { name: '收款核销', source: 'FinanceResourcePage.modules.receivables.tabs.settlement', description: '展示应收的收款和核销记录。', rules: '用于账龄、核销状态和信用释放判断。' },
  { name: '调整记录', source: 'FinanceResourcePage.modules.receivables.tabs.adjust', description: '展示售后退货、退款、折让触发的应收调整节点。', rules: '调整金额、红冲和退款需保留售后来源。' },
];

const payableSourceDetailFields: PrdField[] = [
  { name: '入库记录', source: 'FinanceResourcePage.modules.payables.tabs.inbound / recordRows(row, inbound)', description: '展示采购入库、验收和质检确认记录。', rules: '用于应付暂估和可付款判断。' },
  { name: '应付暂估', source: 'FinanceResourcePage.modules.payables.tabs.source / detailFields(row)', description: '展示尚未到票时按入库或订单形成的暂估明细。', rules: '到票或冲减后需要保留原暂估追溯。' },
  { name: '质检扣款', source: 'recordRows(row, quality)', description: '展示采购退货或质检扣款导致的冲减节点。', rules: '冲减只影响应付汇总和可付款金额。' },
  { name: '三单匹配来源', source: 'FinanceResourcePage.modules.payables.tabs.match', description: '展示采购订单、入库记录、进项发票三方引用。', rules: '三方引用必须同时可追溯。' },
];

const invoiceSourceDetailFields: PrdField[] = [
  { name: '来源明细', source: 'FinanceResourcePage.modules.invoices.tabs.source / recordRows(row, source)', description: '展示销售、采购或售后来源的发票明细。', rules: '开票抬头、税号、金额和来源单据必须一致。' },
  { name: '勾稽记录', source: 'FinanceResourcePage.modules.invoices.tabs.match', description: '展示发票与应收、应付或原发票的勾稽关系。', rules: '同一发票不可重复超额勾稽。' },
  { name: '税务认证', source: 'FinanceResourcePage.modules.invoices.tabs.tax', description: '展示采购进项发票的收票和认证信息。', rules: '认证结果回写应付到票和三单匹配依据。' },
  { name: '红冲记录', source: 'FinanceResourcePage.modules.invoices.tabs.red', description: '展示红冲关联的原发票和处理节点。', rules: '红冲必须勾稽原发票并保留原因。' },
];

const settlementSourceDetailFields: PrdField[] = [
  { name: '核销对象', source: 'FinanceResourcePage.modules.settlements.tabs.source / recordRows(row, source)', description: '展示当前流水可核销的应收、应付或退款对象。', rules: '可核销金额不得超过对象余额。' },
  { name: '银行对账', source: 'FinanceResourcePage.modules.settlements.tabs.bank', description: '展示尚未银行对账或存在差异的流水。', rules: '对账差异未处理时不得自动终态。' },
  { name: '附件回单', source: 'FinanceResourcePage.modules.settlements.tabs.attach', description: '展示流水附件和回单信息。', rules: '附件用于追溯，不替代核销状态。' },
  { name: '凭证记录', source: 'FinanceResourcePage.modules.settlements.tabs.voucher', description: '展示当前流水生成或关联的凭证记录。', rules: '凭证记录只读展示，后续独立凭证模块拆出。' },
];
...
function listDoc(id: string, module: string, title: string, route: string, fields: PrdField[], states: PrdState[], dataSources: string[], sourceText: string): PrdDocument {
  return {
    ...
    scope: ['展示当前模块核心财务对象', `展示来源范围：${sourceText}`, '提供统计卡片和日期筛选', '进入详情或动作处理页', '来源业务明细保持原始追溯关系'],
    dataSources,
    fields,
    states,
    flows: [],
    ...
    validations: ['每条记录必须有来源；无来源记录必须显示补录原因。', '金额字段必须有两位小数并保留负数调整场景。', '筛选项必须覆盖状态机里的所有有效状态。'],
    writeBacks: ['列表页不执行回写，只展示由详情页、动作页或后端任务产生的回写结果。'],
    acceptance: [`用户可以在${title}查看${title.replace('列表', '')}记录、处理进度和风险状态。`, `用户可以按日期、来源范围、状态和关键字段筛选${title.replace('列表', '')}记录。`, '来源单据可点击追溯到对应业务对象。', '统计卡片随当前列表筛选同步更新。'],
...
function createDoc(id: string, module: string, title: string, route: string, states: PrdState[], dataSources: string[], target: string): PrdDocument {
  return {
    ...
    states: [],
    flows: [],
    interactions: ['来源选择弹窗只显示当前模块允许的来源类型。', '选择来源后自动替换明细子表。', '无来源补录必须填写补录原因。', '保存后返回来源动作页或当前模块列表。'],
    validations: ['未选择来源时只能保存为无来源补录。', '来源金额不得超过可处理金额。', '必填附件缺失时阻止提交。', '回写目标必须展示“只回写状态和汇总”。'],
    writeBacks: ['新增页保存草稿不回写业务源；提交确认后由后端按来源对象规则回写来源汇总。'],
    acceptance: [`用户可以通过来源弹窗选择允许的来源单据并带入${target}明细。`, '无来源补录时可以填写补录原因并保存当前记录。', '明细金额、往来单位、来源引用和附件在保存后保持可追溯。', '保存后返回来源动作页或当前模块列表。'],
    sectionIds: ['fields', 'acceptance'],
  };
}

function financeDetailExtraFields(id: string): PrdField[] {
  if (id.startsWith('receivables-')) return receivableSourceDetailFields;
  if (id.startsWith('payables-')) return payableSourceDetailFields;
  if (id.startsWith('invoices-')) return invoiceSourceDetailFields;
  if (id.startsWith('settlements-')) return settlementSourceDetailFields;
  return [];
}
...
function detailDoc(id: string, module: string, title: string, route: string, fields: PrdField[], states: PrdState[], dataSources: string[], target: string): PrdDocument {
  return {
    ...
    dataSources,
    fields: [...fields.slice(0, 8), ...financeDetailExtraFields(id)],
    states,
    flows: [],
    ...
    acceptance: [`用户可以在${title}查看${target}基础信息、来源明细、处理记录、凭证记录和操作日志。`, '来源单据可点击追溯，来源明细展示确认节点。', '用户可以通过状态说明判断当前处理阶段和可执行动作。', '操作完成后状态、金额汇总和处理记录同步回显。'],
    sectionIds: ['details', 'fields', 'states', 'acceptance'],
  };
}
...
function actionDoc(id: string, module: string, title: string, route: string, pageType: string, fields: PrdField[], states: PrdState[], dataSources: string[], action: string, validations: string[], writeBacks: string[]): PrdDocument {
  return {
    ...
    route,
    pageType,
    objective: `处理${title}相关的待办队列，明确处理对象、金额口径、处理结果和回写目标。`,
    scope: ['展示待处理来源清单', '提供新增/处理入口', '记录处理结果', '回写来源状态和汇总'],
    dataSources,
    fields,
    states: [],
    flows: [],
    interactions: ['动作页使用待办列表承接处理对象。', `处理动作说明：${action}`, '卡片统计当前动作相关数据。', '查看操作进入原始财务对象详情。', '处理完成后回到来源财务对象或待办列表。'],
    validations,
    writeBacks,
    acceptance: [`用户可以在${title}查看待处理来源清单并完成${action}`, '单号、来源、往来单位和金额沿用当前处理对象。', '提交处理后展示处理结果、状态变化和回写目标。', '处理完成后可返回来源财务对象或待办列表继续跟进。'],
    sectionIds: ['fields', 'acceptance'],
  };
}
...
function settingDoc(id: string, title: string, route: string, objective: string, fields: PrdField[] = settingFields): PrdDocument {
  return {
    ...
    dataSources: ['财务策略配置', '公共设置母版', '客户管理/销售设置公共交互口径'],
    fields,
    states: [],
    flows: [],
    interactions: ['设置页按财务策略分组展示配置项。', '策略项支持编辑、审批、发布、停用。', '公共字段、编号、审批、打印模板复用公共设置能力。'],
    validations: ['策略发布前校验必填项。', '影响回写或金额的策略必须审批。', '策略停用不得影响历史单据。'],
    writeBacks: ['设置页不回写业务单据，只影响后续单据生成和动作校验。'],
    acceptance: ['用户可以编辑、审批、发布或停用财务策略项。', '设置页按应收、应付、发票和资金策略分组展示配置内容。', '策略说明、适用范围、生效时间和操作记录可追溯。', '发布后的配置能被对应业务页面读取。'],
    sectionIds: ['fields'],
  };
}

const dataSources = {
  workbench: ['应收管理', '应付管理', '发票管理', '收付核销', '销售中心', '采购中心', '售后中心', '仓库中心'],
...
const financeBusinessFlowDoc: PrdDocument = {
  id: 'finance-business-flow',
  module: '财务中心',
  title: '财务中心业务流程 PRD',
  route: '/finance',
  pageType: '中心业务流程',
  objective: '按财务中心统一整理应收、应付、发票、收付核销和财务设置的跨页面业务流转，说明工作台、列表、动作、设置和跨中心回写的衔接关系。',
  scope: ['财务中心级业务流程', '应收应付建账', '发票处理', '收付核销', '跨销售、采购、售后、仓储回写'],
  dataSources: dataSources.workbench,
  fields: [],
  states: [],
  flows: [
    { step: '1', role: '业务系统', action: '销售、采购、售后、仓储或补录来源生成应收、应付、发票或资金处理需求。', output: '财务候选来源。' },
    { step: '2', role: '财务人员', action: '在应收、应付、发票、资金页面复核来源、往来单位、金额、日期、附件和处理条件。', output: '可处理财务记录。' },
    { step: '3', role: '财务人员', action: '完成应收确认、应付确认、付款申请、收付款登记、开票、收票认证、红冲或调整。', output: '财务处理结果。' },
    { step: '4', role: '财务人员', action: '执行核销、三单匹配、发票勾稽、客户对账、供应商对账或银行对账。', output: '核销与对账结果。' },
    { step: '5', role: '系统', action: '只回写来源对象状态、汇总金额、发票状态、核销状态和处理摘要，不修改来源明细。', output: '跨中心财务回写。' },
    { step: '6', role: '系统', action: '刷新财务工作台待办、风险提醒、列表统计和操作日志。', output: '财务中心业务闭环结果。' },
  ],
  interactions: ['从财务工作台、导航分组或列表入口进入流程相关页面。', '流程步骤按应收、应付、发票、收付核销和财务设置顺序展示。', '每个步骤说明负责角色、处理动作和业务输出。'],
  validations: ['流程按来源、金额、状态、账户、发票和核销校验推进。', '跨中心回写必须保留来源对象和操作日志。'],
  writeBacks: ['流程结果回写财务工作台统计、财务对象状态和跨中心处理摘要。'],
  acceptance: ['财务中心业务流程能说明主要页面之间的先后关系。', '工作台待办、列表状态、收付核销和跨中心回写在流程中有对应说明。', '流程覆盖应收、应付、发票、收付核销和跨中心回写闭环。'],
  sectionIds: ['flows'],
};
...
export const financePrdDocuments: PrdDocument[] = [
  {
    ...
    acceptance: ['用户可以在财务工作台查看待办汇总、风险提醒、最近流转和跨中心回写摘要。', '待收款、逾期、待付款、待开票、待收票、退款、调整、三单匹配和银行对账等卡片可进入对应页面。', '风险提醒按金额、到期日和状态排序展示。'],
    sectionIds: ['fields'],
  },
  financeBusinessFlowDoc,
  ...
  actionDoc('invoices-match', '发票管理', '发票勾稽', '/finance/invoices?action=match', '勾稽列表页', invoiceFields, invoiceStates, ['发票记录', '应收单', '应付单', '收付核销记录'], '建立发票与来源财务对象的勾稽关系。', ['同一发票不可重复勾稽超额。', '红冲发票必须勾稽原发票。'], ['回写发票勾稽状态和来源单据发票状态。']),
  ...
  listDoc('settlements-list', '收付核销', '流水列表', '/finance/settlements', settlementFields, settlementStates, dataSources.settlements, '银行流水、收款单、付款单、退款付款单'),
  createDoc('settlements-create', '收付核销', '新增流水', '/finance/settlements?action=new', settlementStates, dataSources.settlements, '资金流水'),
  detailDoc('settlements-detail', '收付核销', '流水详情', '/finance/settlements?id={id}', settlementFields, settlementStates, dataSources.settlements, '资金流水'),
  actionDoc('settlements-receive', '收付核销', '收款登记', '/finance/settlements?action=receive', '动作列表页', settlementFields, settlementStates, ['应收单', '银行收款流水', '客户档案'], '登记客户收款流水并准备核销。', ['收款客户必须匹配应收客户。', '无来源收款只能进入待确认。'], ['回写应收收款状态，核销后释放信用。']),
  actionDoc('settlements-pay', '收付核销', '付款登记', '/finance/settlements?action=pay', '动作列表页', settlementFields, settlementStates, ['应付单', '银行付款流水', '供应商账户'], '登记供应商付款流水并准备核销。', ['付款账户和供应商账户必须校验。', '未审批付款申请不得付款。'], ['回写应付付款状态，核销后更新采购付款状态。']),
  actionDoc('settlements-refund', '收付核销', '退款付款', '/finance/settlements?action=refund', '动作列表页', settlementFields, settlementStates, ['售后退款单', '售后处理方案', '银行付款流水'], '处理售后退款付款。', ['退款未确认付款时不得回写已付款。', '售后关闭需同时满足退货入库、退款付款、应收调整、发票红冲。'], ['付款确认后回写售后退款已付款，未付款前只显示待付款。']),
  actionDoc('settlements-settle', '收付核销', '核销处理', '/finance/settlements?action=settle', '核销工作台', settlementFields, settlementStates, ['资金流水', '应收单', '应付单', '售后退款单'], '将收款、付款、退款流水核销到对应财务对象。', ['流水方向必须和核销对象一致。', '可核销金额不得超过对象余额。', '差异进入差异待处理。'], ['回写应收、应付或售后退款单核销状态。']),
  actionDoc('settlements-bank', '收付核销', '银行对账', '/finance/settlements?action=bank', '对账工作台', settlementFields, settlementStates, ['银行流水导入文件', '系统资金流水', '账户配置'], '匹配银行流水和系统流水。', ['账户、金额、日期、摘要匹配度不足时进入差异待处理。', '未对账流水可核销但必须提示风险。'], ['回写流水对账状态和差异处理结果。']),
...
function treePage(docId: string, label: string, level: PrdTreeNode['level'] = 3): PrdTreeNode {
  const document = financePrdDocuments.find((item) => item.id === docId);
  const sectionIds = document?.sectionIds ?? ['details', 'fields', 'states', 'acceptance'];
...
export const financePrdTree: PrdTreeNode[] = [
  {
    ...
    level: 1,
    children: [
      treePage('finance-workbench', '工作台', 2),
      treePage('finance-business-flow', '业务流程', 2),
      {
