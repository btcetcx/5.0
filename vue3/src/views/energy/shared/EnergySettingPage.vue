...
<template>
  <aw-setting-page class="energy-settings">
    <template #toolbar>
      <aw-setting-toolbar back-text="返回能耗中心" add-text="保存设置" show-add @back="router.push('/energy/monitor')" @refresh="refreshSettings" @add="save" />
    </template>

    <section class="aw-form-card energy-setting-head">
      <h1>能耗中心设置</h1>
      <p>配置采集点、告警阈值、分析维度和碳排放因子。</p>
    </section>

    <div class="aw-tabs energy-settings-tabs">
      <button v-for="t in tabs" :key="t.key" :class="['t', { on: activeTab === t.key }]" type="button" @click="activeTab = t.key">{{ t.label }}</button>
    </div>
...
.energy-setting-head h1 {
  margin: 0;
}

.energy-setting-head p {
  color: var(--aw-fg-3);
  margin: 6px 0 0;
}
